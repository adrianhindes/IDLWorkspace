...
if err ne 0 then begin
   print,'error in imgplot'
   return
endif

;if !d.name eq 'PS'  then begin
;   contourn2,z,x,y,/cb,xr=xr,yr=yr,zr=zr,xsty=xsty,ysty=ysty,position=position,noerase=noerase,pal=pal,title=title,iso=iso,reverse=reverse,nlevels=nlevels,xtitle=xtitle,ytitle=ytitle,offx=offx,ztitle=ztitle
;   return
;endif
...
;stop
ix=linspace(ixa(0),ixa(1),long(ixa(1)-ixa(0)+1))
iy=linspace(iya(0),iya(1),long(iya(1)-iya(0)+1))
