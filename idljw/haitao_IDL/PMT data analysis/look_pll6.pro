pro look_pll6,sh,twin,fr=fr,ylog=ylog,img=img,win=win,f0=f0,bw=bw,iframe=iframe,ccs=ccs,lmax=lmax,doiframe=doiframe,lockavg=lockavg,dolen=dolen,filetype=filetype,rawsh=rawsh,gate=gate
default,lmax,200
nlags=intspace(-lmax,lmax)
default,filetype,'old' 

default,fr,[1,100]
;dir='D:\MDSPLUS_DATA\PLL2_DATA\'
dir='~/pll2/'
if filetype ne 'new' then fil=dir+'pll'+string(sh,format='(I0)')+'.sav' 
if filetype eq 'old' then fil=dir+'data'+string(sh,format='(I0)')+'.save'
if filetype eq 'new' or filetype eq 'old' then $
   restore,file=fil,/verb       ;mdsopen,'pll',sh

if filetype eq 'new2' then begin
   shtab=intspace(449,465)&nt=n_elements(shtab)
   shtab2=91+findgen(nt)

   xshtab=intspace(397,411)&nt=n_elements(xshtab)
   xshtab2=43+2*findgen(nt)
   shtab=[shtab,xshtab]
   shtab2=[shtab2,xshtab2]

   i=value_locate(shtab,sh)
   shsh=shtab2(i)
   if n_elements(rawsh) ne 0 then shsh=rawsh
   fil=dir+'lastrun_daq/pll_'+string(shsh,format='(I3.3)')+'.lvm'
   data=transpose(myread_ascii(fil,data_start=23,delim=string(byte(9))))
;   data=dum.(0)
;stop
endif

t=data(0,*)
if filetype eq 'new' or filetype eq 'new2' then t-=t(0)
dc=data(1,*)
sq=data(2,*)
gate=data(3,*)
;osc=data(4,*)
...
dgate=dgate(idx)
pll0=pll
dpll=[0,pll(1:*)-pll(0:n_elements(pll)-1)]
pll=pll-mean(pll)
...
power1=abs(fft(locksignal))^2
powerpll=abs(fft(pll))^2
pn=n_elements(locksignal)
freq=findgen(pn/2.0)/(pn*deltat)

plot, freq/1000.0, power1(0:n_elements(freq)-1), title='Power spectrum of lock signal',xtitle='Freq/kHz',ytitle='Power',xrange=fr,/ylog
...
;sh=intspace(449,465);32,40)
sh=intspace(397,411);32,40)
nsh=n_elements(sh)
larr=fltarr(nsh)
for i=0,nsh-1 do begin
lmax=25
;look_pll5,sh(i),win=3,iframe=0,ccs=ccs,lmax=lmax,doiframe=1,lockavg=l1;,f0=23e3,bw=3e3
look_pll6,sh(i),[0,2],win=3,ccs=ccs,lmax=lmax,lockavg=l1,filetype='new';,f0=23e3,bw=3e3
