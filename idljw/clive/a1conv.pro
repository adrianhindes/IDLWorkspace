...
pro a1conv,sh,freqshot,tr2,ex,outr=outr,want=want,out2r=out2r,rout=rout,nostop=nostop,lvonly=lvonly
...
restore,file='/home/cmichael/mse_show_result_'+string(sh,format='(I0)')+'.sav'
shr=sh
trr=tr
no:
;tmp=out.yabin

res={v:transpose(out.yabin), rr:out.rr, t:out.xxbin,ix:findgen(25)}

idxt=where(out.xx ge tr(0) and out.xx lt tr(1))

res={v:transpose(out.yy(*,idxt)), rr:out.rr, t:out.xx(idxt),ix:findgen(25)}
...
if i eq i then begin
;   mkfig,'~/fplot.eps',xsize=15,ysize=11,font_size=12
   plot,f,abs(dum),/ylog,xr=[0,30],$
...
if want eq 'dif2conv' then begin
   itfrom=4;6;3
   outr=abs(tmpp2(itfrom,*))
   out2r=atan2(tmpp2(itfrom,*))
endif

if want eq 'modconv' then begin
...
if want eq 'modconv0' then begin
