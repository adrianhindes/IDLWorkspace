...
for ii=0,3 do begin
   ...
   kk=0
   for j0=-m,m do for j1=-m*0,m*0 do for j2=-m,m do for j3=-m,m do begin
      ordc=[j0,j1,j2,j3]
      j0save(kk)=j0
      j1save(kk)=j1
      j2save(kk)=j2
...
;      print,par
   ...
   ordc=[j0save(imin),j1save(imin),j2save(imin),j3save(imin)]*1
   ord(ii,*)=ord(ii,*)+ordc
...
;   par=linfit(en(ii,*),a*en(ii,*)+corr(ii,*)+ord(ii,*),yfit=yfit,chisqr=c)
   ...
   en2f(ii,*)=yfit
   plot,en(ii,*),a*en(ii,*)+corr(ii,*)+ord(ii,*),pos=pos,noer=ii gt 0,title=string(ii,c,ord(ii,0),ord(ii,1),ord(ii,2),ord(ii,3),$
                                                                                   format='(I0," ",G0," ",3(I0," "),I0)'),/yno,psym=-4
print,par
parsave(*,ii)=par
