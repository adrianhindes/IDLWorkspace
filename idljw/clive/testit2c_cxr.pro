...
sh=shf & lam=532.0e-9 ; full sphere/line cal
...
;    simimgnew, img, sh=sh & img=img*1.0
   newdemod, img,cars, nbin=nb,gap=id2,sh=sh,thx=thx,thy=thy,iz=iz,p=p,str=str,sd=sd,demodtype='real4',indexlist=ilist0,lam=lam ,db=db;,/doplot,kx=kx,ky=ky,kz=kz
...
dum=calcfit_cxr(xpar,/plot,res1=res1,res2=res2)


stop
ftol=1e-4
powell,xpar, xi, ftol, fmin, 'calcfit_cxr',iter=iter,itmax=100
dum=calcfit_cxr(xpar,/plot,res1=res1,res2=res2,res3=res3)
writeancal, shnum, xpar
...
str={res1:res1,res2:res2,res3:res3,sh:sh,thx:thx2,thy:thy2}
