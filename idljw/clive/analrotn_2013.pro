...
for i=0,nimg-1 do begin
    ...
    if par.sim eq 0 then img=getimg(r,index=iarr(i),sm=1,pre='run',path='~/kstartestimages')
...
;    demodcs, img,outs, sets,doplot=doplot,zr=[-2,1],newfac=1. ,save={txt:'run',shot:r,ix:iarr(i)},downsamp=sets.pixfringe,override=doplot eq 1;,linalong=45*!dtor;,/noopl
    demodcs, img,outs, sets,doplot=doplot,zr=[-2,2],newfac=0.6 ,save={txt:'run',shot:r,ix:iarr(i)},downsamp=sets.pixfringe,override= doplot eq 1,rfac=sets.rfac;,linalong=45*!dtor;,/noopl
;limg=total(img,2)
