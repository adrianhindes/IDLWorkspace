...
for state=0,nstates-1 do begin
    for i=0,4 do begin
        ...
        if tmp.type eq 'wp' then begin
            ...
            opd=opd(thx,thy,par=par,delta0=par.delta0,k0=k,kappa=kappat)/2/!pi & k/=!radeg 
            cnt=cnt+1
            if i eq 0 then opd=0*opd+0.25
            if not keyword_set(quiet) then print,'thicknessmm=',tmp.thicknessmm,'facetilt:',tmp.facetilt,'opd=',median(opd),'k=',k,'angle=',par.delta0*!radeg
            if opd gt 100 then kappa=kappat
        endif
        if tmp.type eq 'flc' then begin
            opd=tmp.delaydeg/360 + thx*0.
