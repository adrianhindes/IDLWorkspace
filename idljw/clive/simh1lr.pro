...
fp0=(1-lr) * fgaussof(delay,l1,vcore,tcore)
;    endif
ftottmp=(fa0+fp0)               ;/(aa0+ap0+ap1)
ftot=ftottmp    
...
dl=(l0-lref)/lref
ii=dcomplex(0,1)
gamma=exp(2*!pi*ii*N* (0*1+ kappa * (vrotc + dl)) - (!pi*kappa*N)^2 * vth^2)
...
;delay=linspace(4350,4450,151) & nd=n_elements(delay)
par=[vcore,tcore,lr] ;,tedge,vcore2,tcore2,vcore3,tcore3, ap1,aa0_2,aa0_2,tedge,tedge,ap1_2,ap1_3]*1d0
common cbb,cl0,cl1
cl0=l0
cl1=l1
;cl2=l2
...
par2=[vcore,tcore,lr+0.2]
ftot=f(par,delay)
ftot2=f(par2,delay)
plot,delay,abs(ftot),pos=posarr(1,2,0)
oplot,delay,abs(ftot2),col=2

;fref=fgaussof(delay,(l0*lr + l1*(1-lr)),0,0.1) 

plot,delay,atan2(ftot)*!radeg,pos=posarr(/next),/noer
oplot,delay,atan2(ftot2)*!radeg,col=2
...
;      596];5
;nn=4
;for i3=0,1 do for i1=0,nn-1 do for i2=0,nn-1  do begin ;i1+xx,nn do for i3=i2+x;x,nn do for i4=i3+xx,nn do begin
;    iarr(0,cnt)=i1
;    iarr(1,cnt)=i2
;    iarr(2,cnt)=i1+i2
...
;met(cnt,*)=dihes
print,'errors','dv=',dihes(0),'dt=',dihes(1),'drat=',dihes(2)
...
;mine 4400 1100
;eigenvalues:      18.1734  6.61106e-06    0.0210037
;errorsdv=       3.1900092dt=       7.0310368drat=       388.90834
;% Stop encountered: $MAIN$            293 /home/cam112/idl/clive/simh1lr.pro

;ht
;eigenvalues:      6.66174      2.20602   0.00544529
;errorsdv=      0.39714678dt=       13.547737drat=      0.74122645
