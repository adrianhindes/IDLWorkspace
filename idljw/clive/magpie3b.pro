...
xx3=xx1 & for i=0,nx-1 do xx3(i,*)-=xxm(i)
;wset2,0
...
plotm,xx3,pos=posarr(2,2,0)
plot,xxm,thick=2,/noer,pos=posarr(/curr)

ft=xx*0+complex(0,0)
for i=0,511 do ft(i,*)=fft(xx1(i,*))
plot,abs(ft(*,0)),pos=posarr(/next),/noer
plot,abs(ft(*,1)),col=2,/noer,pos=posarr(/curr)
oplot,abs(ft(*,2)),col=3
oplot,abs(ft(*,3)),col=4
oplot,abs(ft(*,4)),col=5

ix=intspace(180,250)
dum=min(abs(ft(ix,1)),imin)
imin=ix(imin) + ishift
i1=findgen(512)
ia=intspace(0,imin-1)
ib=intspace(imin+1,511)
rhoa=abs(i1(ia)-imin)
rhob=abs(i1(ib)-imin)

plot,rhoa,abs(ft(ia,0)),pos=posarr(/next),yr=[0,max(abs(ft(*,0)))],/noer
oplot,rhob,abs(ft(ib,0)),linesty=2

plot,rhoa,abs(ft(ia,1)),pos=posarr(/next),yr=[0,max(abs(ft(*,1)))],/noer,col=2
oplot,rhob,abs(ft(ib,1)),linesty=2,col=2
