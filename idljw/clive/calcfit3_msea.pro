...
function calcfit3_msea,xpar,plot=plot,res1=res1,res2=res2;,par
common cbc, ncalls
...
strtmp=str
applycal_msea, strtmp,xpar


jj=[1]
;jj=[16,22];[28,21]

gencarriers2,sh=sh,th=[thx(iz(0)),thy(iz(1))],kx=kx,ky=ky,dkx=dkx,dky=dky,p=p,str=strtmp,toutlist=toutlist,/noload,/quiet,tkz=kz,nkz=nkz,mat=mat,vth=thv,vkz=kzv,lam=lam,indexlist=ilist0,dmat=dmat,/useindex,kz=kzav
ilist=ilist0
dcar=(dmat) ## transpose([1,0,1,0])
dph=reform(atan2(dcar))
;stop
for i=0,7 do kzv(*,*,i)+=dph(i)/2/!pi ; phase corrected for s1 etc.
;for i=2,3 do begin
...
for k=0,n_elements(jj)-1 do begin
    j=jj(k)
    i0=(where(slist eq istat(0) and ( abs(kzav) gt 10)))(0)
    i=i0
