pro arr,sh,oplot=oplot,col=col
;sh=7366&t0=0.0
...
;rarr=[1200,replicate(r,nimg)]
;idxarr=[0,indgen(nimg)]
nimg=2
...
ay=ny*0.65
if not keyword_set(oplot) then plot,ph1s(*,ay,1)/4*!radeg,yr=[12,24],ysty=1,col=col,xtitle='Horiz along camera in mid plane',ytitle='Offset angle (deg)' $
else $
  oplot,ph1s(*,ay,1)/4*!radeg,col=col
...
cursor,dx,dy,/down
imgplot,transpose(reform(ang2(*,ay,*))),tmy21,indgen(n_elements(ang2(*,0,0))),/cb,pal=-2,zr=[-.2,.2]
...
sh=[7503,7507,7502,7505,7504,7498]
leg=['PF7 Ip=-4 @ 2T','-2','0','+2','+4','PF7 Ip=0 @ 2.5T']
col=[1,2,3,4,5]

!p.thick=2
mkfig,'~/gascal.eps',xsize=10,ysize=8,font_size=7.5
arr,sh(0)
for i=1,4 do arr,sh(i),col=i+1,/oplot
!p.linestyle=2
arr,sh(5),/oplot,col=3
!p.linestyle=0
legend,leg,col=[1,2,3,4,5,3],linesty=[0,0,0,0,0,2],/right
endfig,/gs,/jp
!p.thick=0
;arr,
end
