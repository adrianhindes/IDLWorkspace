...
function calcfit_wb1,xpar,plot=plot,res1=res1,res2=res2,res3=res3,res4=res4,dostop=dostop
...
common cb2, img,cars,gap,nb,mat,thx,thy,iz,p,str,sd,thv,ilist0,isubx,isuby,iz2,lam,wta,test1=test1


strtmp=str
applycal_wb1, strtmp,xpar ;;,test1=test1;;;notenote!!!
...
;i=(where(ilist eq 28))(0)
    if ncalls eq 1 then begin
        ...
        sz=size(ph,/dim)
        if k eq 0 then phja=fltarr(sz(0),sz(1),njj)
        phja(*,*,k)=ph
;        stop
    endif else begin
        ph=phja(*,*,k)
...
;    chisq+=difcc
    ...
    if k eq 2 then res3=dif
    if k eq 3 then res4=dif
    if keyword_set(plot) then begin
        if k eq 0 then pos=posarr(3,4,0)
        imgplot,a1,pos=pos,/cb,title='meas'+string(wta(k),format='(G0)'),noer=k ne 0 
