...
img=imgdark;light;dark;light;dark;dark;light                    ;imgdark ; pass+act
;goto,nopl
...
;dx=xp(4);sz(0)/2
;dy=sz(1) * 0.8;/2
del=[reform(kzv(dx,dy,jj(0:2)))]
cohwhite1=reform(carswhite1(dx,dy,jj(0:2)))
...
;a0=mymgaussfit(pp,a)
;xmach=1e-3
;print,((mymgaussfit(pp,a+[1,0,0,0]*xmach))(0)-a0(0))/xmach,a0(1)
;print,((mymgaussfit(pp,a+[0,1,0,0]*xmach))(0)-a0(0))/xmach,a0(2)
;print,((mymgaussfit(pp,a+[0,0,1,0]*xmach))(0)-a0(0))/xmach,a0(3)
;print,((mymgaussfit(pp,a+[0,0,0,1]*xmach))(0)-a0(0))/xmach,a0(4)
;stop
;ystart=(mygaussfit(x,a))(0,*)

yfit = LMFIT( x, y2, A , CHISQ=chisq , CONVERGENCE=convergence , FUNCTION_NAME='mymgaussfit', ITER=iter, SIGMA=sigma ,itmax=1000,/double,fita=[1,1,1,1],measure_errors=dy2)
apar(dx,dy,*)=a
carsfit(dx,dy,jj)=ir(yfit)
;print,'a=',a
goto,nop
plot,del^2,alog10(abs(ir(y2))),yr=[-2,0],pos=posarr(2,1,0),title=string(dx,dy)
oplot,del^2,alog10(abs(ir(ystart))),col=2
oplot,del^2,alog10(abs(ir(yfit))),col=3

plot,del,(atan2(ir(y2))),pos=posarr(/next),/noer,yr=[-!pi,!pi]
oplot,del,(atan2(ir(ystart))),col=2
oplot,del,(atan2(ir(yfit))),col=3
nop:
;stop
...
aparbg=apar
save,aparbg,intbg,file='~/idl/clive/settings/aparbg1.sav',/verb

;gam=

bbb:
contrastw=abs(carswhite)
phasew=atan2(carswhite)
;cursor,dx,dy,/down
...
oplot,abs(kzv(dx,dy,inz))^2,alog10(contrastfit(dx,dy,inz)),col=2
oplot,abs(kzv(dx,dy,inz))^2,alog10(contrastw(dx,dy,inz)),col=3
plot,kzv(dx,dy,inz),phase(dx,dy,inz),psym=-4,pos=posarr(/next),/noer,yr=[-1,1]*!pi,xtitle='N',ytitle='phase (rad)'
oplot,kzv(dx,dy,inz),phasefit(dx,dy,inz),col=2
oplot,kzv(dx,dy,inz),phasew(dx,dy,inz),col=3
