...
;        if i eq 0 then     plot,lam,d else oplot,lam,d,col=i+1
    ...
    if qty eq 'int' then begin
        ...
        if k eq 0 and tp eq 0 then $
          plot,fc*ff,d2(*,value_locate(lam0,l0(k))),xtitle='% (2*rms) beam voltage ripple',ytitle=textoidl('polarized intensity @ \lambda=central \sigma'),psym=psarr(tp),title='Investigation of effect of beam ripple',yr=yr1,ysty=1,thick=thk $
         else $
...
legendarr,['ch#1','ch#17','ch#29'],['sin','spiky'],col=[1,2,3],linesty=[0,1],psym=psarr,right=qty eq 'int'
endfig,/gs,/jp
