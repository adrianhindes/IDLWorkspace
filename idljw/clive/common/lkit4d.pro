...
default,extr,''
if filt eq 1 then filttxt='_filtered' 
if filt eq 2 then filttxt='_filteredg'
if filt eq 0 then filttxt=''
...
mkfig,'~/figs/cmpfilt_hires_ripple_scan_'+qty+filttxt+'.eps',xsize=12,ysize=9,font_size=10
rcha=fltarr(3)
for tp=0,1 do begin
...
;        if tp eq 4 and i gt 0 then extr='_hsi'
        ...
        getdat,tab(i),lam,d,2,qty=qty,filt=k,rch=rchdum,extr=extr
        rcha(k)=rchdum
;        if i eq 0 then     plot,lam,d else oplot,lam,d,col=i+1
    ...
    if qty eq 'int' then d2=d2/d2(0,value_locate(lam0,l0(2))) * 100
;    d2=d2*100
    ...
    if qty eq 'int' then begin
        ...
        if k eq 0 and tp eq 0 then $
          plot,fc*ff,d2(*,value_locate(lam0,l0(2))),xtitle='% (2*rms) beam voltage ripple',ytitle=textoidl('Degradation of polarization fraction (rel. to no ripple'),psym=psarr(tp),title=textoidl('\lambda= central \sigma'),yr=yr1,ysty=1,thick=thk $
         else $
          oplot,fc*ff,d2(*,value_locate(lam0,l0(2))),col=k+1,psym=psarr(tp),linesty=tp,thick=thk
    endif 
...
;['ch#1','ch#17','ch#29']
legendarr,['no filt','filt 1.2A FWHM lor','filt 1.2A FWHM gauss'],['sin','[given waveform]'],col=[1,2,3],linesty=[0,1],psym=psarr,/bottom
endfig,/gs,/jp
