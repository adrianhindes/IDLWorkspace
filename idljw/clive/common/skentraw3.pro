function skentraw3, matr, rcoeffr,testt=testt,acoefft=acoefft,dchi=dchi,watch=watch,$
...
sz=size(matr,/dim)
tmatr=fltarr(sz(1),sz(0),sz(2))
for i=0,sets.nf/2-1 do tmatr(*,*,i)=transpose(matr(*,*,i))
na=n_elements(matr(*,0,0))
nm=n_elements(matr(0,*,0))
...
while convg eq 0 do begin
    ...
    grads=fltarr(na,sets.nf/2) & gradc=grads & ent=0. & chisq=0.
    for ii=0,sets.nf/2-1 do begin
        matr_s = matr(*,*,ii) & rcoeff_s = rcoeffr(*,ii) & tmatr_s=tmatr(*,*,ii)
        dfunc,acoefft(*,ii),grads=gradsp,gradc=gradcp,$
               ent=entp,chisq=chisqp
        grads(*,ii)=gradsp
        gradc(*,ii)=gradcp
        ent=ent+entp
        chisq=chisq+chisqp
    ...
    if (ent le entlast) and (i ge itchisq+100) and (itchisq ne 0) then begin
        print, 'ent > entlast so terminating',i,itchisq,ent,entlast
        convg=1
    endif
...
;    a1=multsp(tmatr,e1,sets.nf/2)

    e3= (2*multsp(matr,multsp(tmatr,e1,sets.nf/2),$
                  sets.nf/2))*acoefft^2   

    e4= (2*multsp(matr,multsp(tmatr,e2,sets.nf/2),$
                  sets.nf/2))*acoefft^2   
...
;    e = [[e1],[e2],[e5]]
;    ns=3
    smu = fltarr(ns) & for j=0,ns-1 do smu(j)=total(grads*e(*,*,j))
    cmu = fltarr(ns) & for j=0,ns-1 do cmu(j)=total(gradc*e(*,*,j))
    gmunu = fltarr(ns,ns)
    mmunu = fltarr(ns,ns)
    gmunu2 = fltarr(ns,ns)
    mmunu2 = fltarr(ns,ns)
    for al=0,ns-1 do for be=0,ns-1 do begin
        ...
        compcb = multsp(tmatr,e(*,*,be),sets.nf/2)
        compca = multsp(tmatr,e(*,*,al),sets.nf/2)
...
;    if it ge 50 then stop
    ...
    diff = e1 & diff(*,*)=0.
    for ii=0,ns-1 do diff = diff + e(*,*,ii) * x(ii)
    ...
    if (watch gt 0) then if ((i mod watch) eq 0 ) and (i gt 0) then begin
        !p.multi=[0,5,3]
        for ii=0,sets.nf/2-1 do begin
            contourn2,alog10(reform(acoefft(*,ii),sets.nx,sets.ny)),sets.kx,sets.ky,/nice
            plot,rcoeffr0(*,ii)
            dum=acoefft(*,ii)
            dum1=transpose(matr0(*,*,ii))
