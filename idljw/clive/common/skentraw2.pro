function skentraw2, matr, rcoeffr,testt=testt,acoefft=acoefft,dchi=dchi,watch=watch,$
...
;        convg=1
    ...
    entlast=ent
    e1=grads*(acoefft)
    e1=e1/sqrt(total(e1^2/acoefft))

    e2=gradc*(acoefft)^2
    e2=e2/sqrt(total(e2^2/acoefft))

    e3= (2*matr # (tmatr # e1))*acoefft   

    e4= (2*matr # (tmatr # e2))*acoefft   

;    e5 = e3 - e4
;    e5 = e5 /sqrt(total(e5^2))

    e3=e3/sqrt(total(e3^2/acoefft))
    e4=e4/sqrt(total(e4^2/acoefft))
    e = [[e1],[e2],[e3],[e4]]
    ns=4
;    e = [[e1],[e2],[e5]]
;    ns=3
    smu = grads # e
    cmu = gradc # e
    gmunu = fltarr(ns,ns) ;; this is for gradgrad S
    lmunu = fltarr(ns,ns) ;; this is for the "distance metric"
    mmunu = fltarr(ns,ns)
    gmunu2 = fltarr(ns,ns)
    mmunu2 = fltarr(ns,ns)
    for al=0,ns-1 do for be=0,ns-1 do begin
        dum1=e(*,al) * e(*,be)/acoefft
        dum2=dum1/acoefft
        lmunu(al,be) = total(dum1)
        gmunu(al,be) = total(dum2)
        compcb = tmatr # e(*,be)
        compca = tmatr # e(*,al)
...
;        w(idx)=0.
    ...
    lmunuppp = transpose(v) # diag(sqrt(wi)) # transpose(u) # $
               lmunu # u # diag(sqrt(wi)) # v
    lmu = diag(lmunuppp)
    zi=1/z
    ...
    common alfunc, smupppb,cmupppb, zb, idxb,chisqb, caimtb,penb,lmub
    smupppb=smuppp & cmupppb=cmuppp & zb=z & idxb=idx & chisqb=chisq&caimtb=caimt & lmub = lmu

    xg=max(z);norm(transpose(cmuppp))/norm(transpose(smuppp))
    pen=0.
    l2thres=0.3*total(acoefft)
    l2=1e11
    pmax=1e15
    pmin=0.
    ahigh=0.9e15
...
ret2:
    penb=pen
    dummy=alfun2(alnew,xppp=xppp,cpen=cpen,cee=c)
    l2 = total(lmu*xppp *xppp)
...
;    if it ge 50 then stop
    ...
    print,'chisq,ent,l2,test,caim,it'
    print,chisq,ent,l2,test,caim,it
    print,'cmin,caimt,cnew',cmin,caimt,cnew
