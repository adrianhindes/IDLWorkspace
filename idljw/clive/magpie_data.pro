...
probe2_file = path+'/probe_vplus/'+string(shot_number, format='(i05)') + '.bin'
probe2_file = path+'/probe_isat_rot/'+string(shot_number, format='(i05)') + '.bin'
...
if STRLOWCASE(diag) eq strlowcase('probe_isat') then begin
  if file_test(probe1_file) then begin
    ...
    return, result  
  endelse
endif else if STRLOWCASE(diag) eq strlowcase('probe_vfloat') or STRLOWCASE(diag) eq strlowcase('probe_vplus') or STRLOWCASE(diag) eq strlowcase('probe_isat_rot') then begin
  if file_test(probe2_file) then begin
