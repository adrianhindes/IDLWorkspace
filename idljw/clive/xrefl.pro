...
save,lns,fcb,file=path+fil+'ref'+'show.sav',/verb
plot,fcb0(0,*,*),fcb0(2,*,*),psym=3,/iso
oplot,fcb(0,*,*),fcb(2,*,*),psym=3,col=2
...
if ev.id eq idx.go then begin
widget_control,idx.cent,get_value=cent
widget_control,idx.ang,get_value=ang
endif
...
pro xrefl
common cbxrefl, idx
idx={top:0L,cent:0L,ang:0L,go:0L}

idx.cent=cw_array(idx.top,title='cent rpz',value=[1506,-7.2,519.6])
idx.ang=cw_array(idx.top,title='tilt rot',value=[35., 0.])
idx.go=WIDGET_BUTTON(idx.top,value='Go')
widget_control,idx.top,/realize
xmanager,'xrefl',idx.top,/no_block
