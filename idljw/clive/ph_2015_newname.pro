...
;img=getimgnew(sh,db=db,ifr,info=info,/getinfo,/nostop,/nosubindex,str=str)*1.0
   ...
   circ=0.5*!radeg*atan(abs2(cars(*,*,ic)/carsr(*,*,ic))/ (abs(cars(*,*,ia))+abs(cars(*,*,ib))) ) 
...
for i=0,nsh-1 do for j=0,nang-1 do begin
   ...
   c=linang(j)*!dtor
   q=qwp_ang(i)*!dtor
    svec=[1,cos(2*(d - q))*cos(2*(-c + q)),-(cos(2*(d - q))*sin(2*(-c + q))),sin(2*(d - q))]

;   chi[i,j]=atan(sqrt(svec(2)^2+svec(3)^2)*sgn(svec(2)) , svec(1) )*!radeg*2 
    dum=sgn(1)
   chi[i,j]=atan(sqrt(svec(2)^2+svec(3)^2) , svec(1) )*!radeg*2 
   delta[i,j]=atan(svec(3),svec(2))*!radeg

    theta[i,j]=atan(svec(2),svec(1))*!radeg
