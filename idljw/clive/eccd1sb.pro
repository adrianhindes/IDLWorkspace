...
sh=9323 & freqshot=2. & tr2=[2,6.9751];0+[2,2.9751];[2,6.975];[4.5,6.475];[2,3];7]
...
;  if a ne '' then stop
   ...
   amp3(i)=dum(0)
   dum2=dum*0 &
   for iharm=0,5 do dum2(isel*iharm)=dum(isel*iharm)
   for iharm=15,19 do dum2(isel*iharm)=dum(isel*iharm)
   tmp2(*,i)=float(fft(dum2,/inverse))
;stop
endfor
...
for i=0,nt-1 do begin
   tmpvar=nonans(tmp2(i,*));smooth(nonans(tmp2(i,*)),5);3)
   current(i,*)=-deriv(tmpvar)
endfor

contourn2,tmp2(*,ix),res.t,res.r1(ix),zr=[-20.,20.],pos=posarr(1,4,0,cny=0.1,cnx=0.1),pal=15,offx=1,nl=30,/box,$
...
contourn2,current(*,ix),res.t,res.r1(ix),pos=posarr(/next),pal=-2,offx=1,/cb,/box,$
        title='current',ytitle='-R (cm)',xr=tr(0)+[0,0+2./freqshot],/noer,zr=[-.1,.1]*5
...
plot,res.t,sumc,xr=!x.crange,pos=posarr(/curr),col=2,/noer,ysty=4,xsty=4
