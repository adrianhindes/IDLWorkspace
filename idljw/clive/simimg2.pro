...
sd=100
simimg,            res=res,s3=0.,s2=1.,s1=0.,dd=2.*!dtor*randomn(sd),de=2.*!dtor*randomn(sd),dc=2.*!dtor*randomn(sd),db=2.*!dtor*randomn(sd)
;imgplot,res,/iso
;stop
fr=fft(res)
fr=shift(fr,n/2,n/2)
...
dda=da(0.)
ddac=dda(nx/2,ny/2)
contourn2,(dda-ddac)*!radeg,pal=-2
stop
...
s1=cos(2*ang)
s2=sin(2*ang)
simimg,            res=res,s3=0.,s2=s2,s1=s1,dd=0*!dtor*randomn(sd),de=0.*!dtor*randomn(sd),dc=0.*!dtor*randomn(sd),db=5*!dtor ;*randomn(sd)
;imgplot,res,/iso
;stop
;fr=fft(res)
;fr=shift(fr,n/2,n/2)
