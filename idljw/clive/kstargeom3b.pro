;this version using some #s from marks 'kstar2013.iam'
...
;b0a=[rt,sqrt(-rt^2+2009.^2),0] ; marks aim point
b0a=[rt,sqrt(-rt^2+2070.^2),0]
...
b0=b0 + dvec
b1=b1+dvec
b0=[815,1905.,0] ; this is the incorrect aiming point used to calc 11.5deg
...
;p0=[-982.,2576,275]
p0=[-1063, 2607, 298.] ; point on mirror
...
vliner2 = vline - 2 * (total(vline*mvec)) * mvec

bhatr2 = bhat - 2 * (total(bhat*mvec)) * mvec
print,'vilt of vertical line is',atan(total(bxhat * vliner2),vliner2(2))*!radeg
...
ang=acos(total(zhat * zhatr) / sqrt(total(zhat^2)) / sqrt(total(zhatr^2)))*!radeg
print, 'angle between incidenent and reflected axes is',ang
print,'so mirror angle should be',ang/2
print,'or ,',90-ang/2
print,'or, ',360. - (90-ang/2)
mvecp = [total(mvec * bxhat),total(mvec*[0,0,1])]
mvecp/=total(mvecp^2)

;angtilt=acos(total(mvec * [0,0,1]) / sqrt(total(mvec^2)))*!radeg
angtilt=atan(mvecp(1),mvecp(0))*!radeg

print,'mirror tilt angle is',angtilt
