...
common cb, a,x,wt
common cb2, coherror
common cb3, val_apply_ctedge
common cbb,cl0,cl1,cl2,ctedge
forward_function f
if n_elements(coherror) eq 0 then coherror=0.
f1=f(a+da,x)
if n_elements(val_apply_ctedge) ne 0 then begin
   ctedge_rememb=ctedge
   ctedge=val_apply_ctedge
 ;   stop
endif
f0=f(a,x)

if n_elements(val_apply_ctedge) ne 0 then begin
   ctedge=ctedge_rememb
endif
csq=total( wt^2 * abs(f1-f0+coherror)^2 )
;stop
return,csq
end
...
fha=lr2 *  fgaussof(delay,l2,v2,tedge)
;print,'tedge=',tedge
;if tedge gt 1 then stop
;    endif
...
;from haitao
;P (0)+W(!pi/4, Linbo3,20mm)+ W(!pi/4, bbo,1mm)+ d(!pi/4, bbo,5mm)+AHWP(!pi/8)+ W(-!pi/4, Linbo3,7.5mm)+ d(-!pi/4, bbo,5mm)+P (0)
...
delay=linspace(0,9000,151) & nd=n_elements(delay)
par=[vcore,tcore,lr,lr2,v2] ;,tedge,vcore2,tcore2,vcore3,tcore3, ap1,aa0_2,aa0_2,tedge,tedge,ap1_2,ap1_3]*1d0
common cbb,cl0,cl1,cl2,ctedge
...
;delay1=2850.;4100.;dinc[iarr(0,cnt)]
;delay2=3950.;550.*2;dinc[iarr(1,cnt)]
delay1=delay1ht
delay2=delay2ht
...
met(cnt,*)=dihes
print,'errors','dv=',dihes(0),'dt=',dihes(1),'drat=',dihes(2),'drat2=',dihes(3),'dv2=',dihes(4)
...
print,'opt delay1=',iarr(0,imax)*dinc,'delay2=',iarr(1,imax)*dinc
;stop


dpar=[0.01,1,0.1,0.05,-0.005]
dpar*=0

common cb3, val_apply_ctedge
val_apply_ctedge=0.001
csq1=csq(dpar)
...
;stop
xi=diag_matrix([.01,1,0.1,0.1,0.01])
ftol=1e-4

dpar1=dpar
;common cb2, coherror
;coherror=1*!dtor/1695. * vec *exp(complex(0,1)*randomu(sd,4)*2*!pi)


powell,dpar1,xi,ftol,fmin,'csq',iter=iter
print,fmin
print,dpar1*[100,10,1,1,100]
stop
...
; print,delay1ht,delay2ht
;       3872.8749       1695.3913

;eigenvalues:      4.02846      2.05141      1.05044     0.516025   0.00295207
;errorsdv=      0.62074617dt=       18.389631drat=      0.70000149drat2=      0.92856557dv2=       1.5666956


; print,delay1,delay2
;      3200.00      3400.00

;eigenvalues:      17.5349      2.54803     0.718669     0.175179    0.0118389
;errorsdv=      0.24008499dt=       9.1570996drat=       2.4204893drat2=      0.90273892dv2=       1.1970584
