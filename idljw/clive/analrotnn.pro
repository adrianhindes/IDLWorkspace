...
pars2={nimg:2,$
       ...
       sim:3,$
       opt:'a',$
;       lambdaleft:[656e-9,655.5e-9],$
;       lambdaright:[656e-9,656.5e-9]}

       lambdaleft:[659.89e-9,660e-9],$
       lambdaright:[659.89e-9,661e-9]}
...
for i=0,nimg-1 do begin
    ...
    img=simimg_p3n(s0(i),s1(i),s2(i),s3(i),mode=par.mode,opt=par.opt,lambdaleft=par.lambdaleft(i),lambdaright=par.lambdaright(i),dosynth=0,/lamimage)
...
;    demodcs, img,outs, sets,doplot=doplot,zr=[-2,1],newfac=1. ,save={txt:'run',shot:r,ix:iarr(i)},downsamp=sets.pixfringe,override=doplot eq 1;,linalong=45*!dtor;,/noopl
    demodcs, img,outs, sets,doplot=doplot,zr=[-2,2],newfac=1. ,save={txt:'run',shot:r,ix:iarr(i)},downsamp=sets.pixfringe,override= 1,rfac=sets.rfac;,linalong=45*!dtor;,/noopl
;limg=total(img,2)
...
;    zrr=[-.1,.1]
    ...
    imgplot,ph1,pos=pos,/cb,title='1',pal=-2
    imgplot,ph2,/noer,pos=posarr(/next),/cb,pal=-2,title='2'
    imgplot,phs,/noer,pos=posarr(/next),/cb,pal=-2,title='sum'
    imgplot,phd,/noer,pos=posarr(/next),/cb,pal=-2,title='diff'
