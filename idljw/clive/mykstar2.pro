...
dopl=0
fracbw=1.0
demodc, img,c1,c2a,c2b,s3,idxng=idx,thres=0.1,pixfringe=10,aoffs=aoffs,wintype='sg',sgmul=sgmul,sgexp=sgexp,dopl=dopl,typthres='data',fracbw=fracbw
...
if n_elements(uc) eq 0 then uc = get_kstar_mse_images(shotnoc, camera=camera,cal=1, time=time, tree=tree)

if n_elements(uc2) ne 0 then dum=temporary(uc2)
if n_elements(uc2) eq 0 then uc2 = get_kstar_mse_images(shotnoc2, camera=camera, time=time, tree=tree)
...
sk:
;ucf=uc
demodc, ucf,c1r,c2ar,c2br,s3r,idxng=idx,thres=0.1,pixfringe=10,aoffs=aoffs,wintype='sg',sgmul=sgmul,sgexp=sgexp,dopl=dopl,zr=zr,/noopl,typthres='data',fracbw=fracbw
