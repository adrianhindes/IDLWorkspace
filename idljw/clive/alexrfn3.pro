...
default,doplot,0
db='h1tor'
fil='/alex/background 2015 April 15 11_18_39.spe'
read_spe,fil,l,t,dback

dback=dback*1.0
if not keyword_set(sim) then d=getimgnew(sh,0,db=db,info=info,/getinfo)*1.0 - dback*0. else simimgnew,d,sh=sh,db=db,svec=[1,0,1,0]
gatedelay=info.gatedelay
demodtype='basicrf1n3'
...
p=p(idx)
gd=fltarr(nn)
;sh0=85640 ;earlier, no puff (during shot)
sh0=85740+nshift; puffer
for i=0,nn-1 do begin
getzeta,(sh0+idx(i)),zeta1,light1,raw1,iwy=1,gatedelay=gg& z1(*,*,i)=zeta1 & l1(*,*,i)=light1&r1(*,*,i)=raw1 & gd(i)=gg
endfor
e:
;plotm,z1;,xr=[200,350]
zav=total(z1,3) / nn
...
xr=[0,1000]
yr=[600,900]
imgplot,alog10(zav),pos=posarr(/curr),/noer,/cb,zr=[-2,-0.5],/iso,xr=xr,yr=yr,xsty=1,ysty=1
contour,lav,pos=posarr(/curr),lev=linspace(0.2,0.6,5),/noer,/iso,xr=xr,yr=yr,xsty=1,ysty=1

imgplot,rav,/cb,xr=xr,yr=yr,xsty=1,ysty=1,zr=[0,2],/iso,pos=posarr(/curr)
...
imgplot,(abs(sz2s)),pos=posarr(/next),/noer,/cb,zr=[0,.01],/iso,xr=xr,yr=yr,xsty=1,ysty=1
stop
contour,lav,pos=posarr(/curr),lev=linspace(0.2,0.6,5),/noer,/iso,xr=xr,yr=yr,xsty=1,ysty=1
