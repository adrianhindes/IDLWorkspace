...
readpatch,9997,db='h1tor',str,nfr=1
if n_elements(ix1) ne 0 then dum=temporary(ix1)
if n_elements(iy1) ne 0 then dum=temporary(iy1)
nl=201
getptsnew,pts=pts,str=str,bin=8,/pptsonly,lene=[0,200.],nl=nl,nx=nx,ny=ny,detx=detx,dety=dety,ix=ix1,iy=iy1,leno=len1
dl = 0.01
...
ss=myrest2('~/idl/clive/nleonw/aview/objhidden2_zb.sav')
...
inten = inten * ixx ; mask where path length is less than obstruction
...
;imgplot,linten2,ix1,iy1,pos=posarr(3,1,0),/iso,title='calculated image, intenisty=ring round rho=.8'




dir='/data/kstar/halpha/'
fil='shaun_84081.spe'


read_spe,dir+fil,l,t,im & im=im*1. ; 5ms exposure time gain 1

im=(myrest2('~/hcal.sav')).(0)

im=rotate(im,2)

mkfig,'~/torha.eps',xsize=22,ysize=19,font_size=10

contourn2,(im)/1e17,pos=posarr(2,2,0),/iso,noer=0,title=textoidl('measured H_{\alpha}'),/cb,zr=[0,1.5],offx=1,ztitle=textoidl('10^{17} ph/s/m^2/str'),xsty=5,ysty=5

contour,linten2,ix1,iy1,pos=posarr(/curr),/noer,xr=!x.crange,yr=!y.crange,nl=10,/iso,xsty=5,ysty=5


contourn2,linten2/1e17,ix1,iy1,pos=posarr(/next),/cb,/noer,zr=[0,1.5],title=textoidl('Modeled H_{\alpha}'),offx=1,ztitle=textoidl('10^{17} ph/s/m^2/str'),/iso,xsty=5,ysty=5

im=read_png('~/idl/clive/nleonw/aview/cr4.png')*1.0 & im=rotate(im,2)
imgplot,alog10(im),zr=alog10([1000,60000]),pos=posarr(/next),/iso,/noer,title='survey img',/cb,xsty=5,ysty=5

endfig,/gs,/jp
;imgplot, zb, ix1, iy1, zr=[0,3000],pos=posarr(/next),/noer,/iso,/cb
;contour, zb, ix1, iy1, lev=linspace(0,3000,11),pos=posarr(/curr),/noer,xr=!x.crange,yr=!y.crange,xsty=1,ysty=1,/iso
...
dum=reform(inten,nx,ny,201)  
;imgplot,reform(dum(*,ny*0.25,*)),/iso,pos=posarr(/next),/noer
dum=reform(ixx,nx,ny,nl)
;imgplot,reform(dum(*,ny*0.15,*)),/iso,pos=posarr(/curr),/noer
