...
twin=[10,15,20,25,30,35,40.]*1e-3
nsh=n_elements(twin)
sh=85989*replicate(1,nsh)
vsep=-0.3
...
colarr=[1,2,3,4,5,6,8,9,10,11,12]
for i=0,nsh-1 do begin
   mode='mirnov'
   mode='isat'
   tr=[.03,.04]
...
f=fft_t_to_f(tt)
;
if i eq 0 then    plot,f/1e3,alog10(s1s),xr=[1000,100e3]/1e3,xtitle='Freq/kHz',ytitle='Log power',yr=yr,/xlog,thick=2 else oplot,f/1e3,alog10(s1s)+i+vsep,col=colarr(i),thick=2
