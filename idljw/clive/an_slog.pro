...
for i=0,nsh-1 do begin
   mode='isat'
   tr=[.03,.04]
   doit,sh(i),ff=f,s1s=s1s,mode=mode,/just,st=st1,mn=mn1,val=val1,tr=tr
   mn(i)=mn1
   st(i)=st1

;,dostop=i eq 3
   doit,sh(i),mode='lint',/just,st=st1,mn=mn1,tr=tr,/only1

   mn2(i)=mn1
   st2(i)=st1

;   stop
   if mode eq 'vfloat' then yr=[-6,2] else yr=[-12,-6];yr=[-14,-3]
;dum=temporary(yr)
s1s=abs(fft(val1))^2
nsm=5
s1s=smooth(s1s,nsm)
