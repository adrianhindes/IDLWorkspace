...
FUNCTION jw_bes_xcorr, shot=shot, ch1=ch1, ch2=ch2, ch1_array = ch1_array, ch2_array = ch2_array,$
  trange=trange , freq_filter = freq_filter, subwindow_npts = subwindow_npts, $
  overlap = overlap, han = han, $
  plot = plot, filter_plot = filter_plot, hungary = hungary, dt=dt, fast = fast
...
;    reform_channel1 = reform(JW_BANDPASS(channel1(0:subwindow_number*subwindow_size-1), freq_filter[0], freq_filter[1] , BUTTERWORTH=50.0),[subwindow_size,subwindow_number])
;    reform_channel2 = reform(JW_BANDPASS(channel2(0:subwindow_number*subwindow_size-1), freq_filter[0], freq_filter[1] , BUTTERWORTH=50.0),[subwindow_size,subwindow_number])
...
;    endfor
    ...
    ch_var = dblarr(2)
    ch_var[0] = variance(channel1)/mean(channel1)
    ch_var[1] = variance(channel2)/mean(channel2)
;    print, 'channel1 variance: ' , ch_var[0]
;    print, 'channel2 variance: ' , ch_var[1]
  endif else begin
    subwindow_size = subwindow_npts
    subwindow_number = floor(size(channel1.data,/N_ELEMENTS)/subwindow_size)
;    print, subwindow_size
;    print, 'subwindow_number: ', subwindow_number
    reform_channel1 = reform(channel1.data(0:subwindow_number*subwindow_size-1),[subwindow_size,subwindow_number])
    reform_channel2 = reform(channel2.data(0:subwindow_number*subwindow_size-1),[subwindow_size,subwindow_number])
;    ;;;;;
;    reform_channel1 = reform(channel1.data(0:subwindow_number*subwindow_size-1)/mean(channel1.data(1:subwindow_number*subwindow_size)),[subwindow_size,subwindow_number])
;    reform_channel2 = reform(channel2.data(0:subwindow_number*subwindow_size-1)/mean(channel2.data(1:subwindow_number*subwindow_size)),[subwindow_size,subwindow_number])
;    reform_channel1 = reform(JW_BANDPASS(channel1.data(0:subwindow_number*subwindow_size-1), freq_filter[0], freq_filter[1] , BUTTERWORTH=50.0),[subwindow_size,subwindow_number])
;    reform_channel2 = reform(JW_BANDPASS(channel2.data(0:subwindow_number*subwindow_size-1), freq_filter[0], freq_filter[1] , BUTTERWORTH=50.0),[subwindow_size,subwindow_number])
    for i = 0L, subwindow_number-1 do begin
      reform_channel1(*,i) = JW_BANDPASS(reform_channel1(*,i), freq_filter[0], freq_filter[1] , BUTTERWORTH=50.0)
      reform_channel2(*,i) = JW_BANDPASS(reform_channel2(*,i), freq_filter[0], freq_filter[1] , BUTTERWORTH=50.0)
    endfor
    ...
    ch_var = dblarr(2)
    ch_var[0] = variance(channel1.data)/mean(channel1.data)
    ch_var[1] = variance(channel2.data)/mean(channel2.data)
;    print, 'channel1 variance: ' , ch_var[0]
;    print, 'channel2 variance: ' , ch_var[1]
  endelse
...
xcorr_lag = lindgen(floor(subwindow_size/fast)*2-1)-(floor(subwindow_size/fast)-1)
correlation12 = dblarr(subwindow_number,floor(subwindow_size/fast)*2-1)
for i = 0L, subwindow_number-1 do begin
  correlation12(i,*) = yc_correlate(reform_channel1(*,i),reform_channel2(*,i),xcorr_lag,unbiased=unbiased)
endfor
...
fit_points = [where((xcorr_lag LE -1)*(xcorr_lag GE -2)), where((xcorr_lag GE 1)*(xcorr_lag LE 2)) ]

hilb = dblarr(size(correlation12,/dimensions))
for i = 0L, subwindow_number-1 do begin
  if array_equal(ch1_array, ch2_array) then begin
    corr_fit = jw_quad_fit(xcorr_lag[fit_points],correlation12[i,fit_points] )
    corr_value[0,i] = corr_fit[2]
    corr_value[1,i] = corr_fit[2]
    mean_xcorr02[where(xcorr_lag EQ 0)] = corr_value[0,i]
    mean_envelope02[where(xcorr_lag EQ 0)] = corr_value[1,i]
;    ycplot, corr_value[0,*], out_base_id = oid
;    ycplot, corr_value[1,*], oplot_id = oid 
  endif else begin
...
;ycplot, xcorr_lag*dt*1e6, mean_xcorr02, out_base_id = oid
;ycplot, xcorr_lag[where((xcorr_lag GT -10) and (xcorr_lag LT 10))]*dt*1e6, corr_fit[0]*(xcorr_lag[where((xcorr_lag GT -10) and (xcorr_lag LT 10))]-corr_fit[1])^2+corr_fit[2], oplot_id = oid
;ycplot, xcorr_lag*dt*1e6, mean_envelope02, oplot_id = oid

;stop

corr_value2 = dblarr(2)
corr_std = dblarr(2)
if array_equal(ch1_array, ch2_array) then begin
  corr_value2[0] = mean(corr_value[0,*])
  corr_std[0] = stddev(corr_value[0,*])
  corr_value2[1] = corr_value2[0]
  corr_std[1] = corr_std[0]
  
  corr_mean = dblarr(size(correlation12[1,*],/dimensions))
  loop_size = size(xcorr_lag)
  for i= 0L, loop_size[1]-1 do begin
    corr_mean[i] = mean(correlation12[*,i])
  endfor
;  ycplot, xcorr_lag, corr_mean, out_base_id = oid
endif else begin
  corr_value2[0] = mean(correlation12[*,where(xcorr_lag EQ 0)])
  corr_std[0] = stddev(correlation12[*,where(xcorr_lag EQ 0)])
  hilb_mean = dblarr(size(hilb[1,*],/dimensions))
  corr_mean = dblarr(size(correlation12[1,*],/dimensions))
  loop_size = size(xcorr_lag)
  for i= 0L, loop_size[1]-1 do begin
    hilb_mean[i] = mean(hilb[*,i])
    corr_mean[i] = mean(correlation12[*,i])
  endfor
  envelope02 = sqrt(hilb_mean^2.0 + corr_mean^2.0)
  
;  ycplot, xcorr_lag, corr_mean, out_base_id = oid
;  ycplot, xcorr_lag, envelope02, oplot_id = oid
  
  hilb_var = mean(hilb[*,where(xcorr_lag EQ 0)]^2)-mean(hilb[*,where(xcorr_lag EQ 0)])^2
  original_var = mean(correlation12[*,where(xcorr_lag EQ 0)]^2)-mean(correlation12[*,where(xcorr_lag EQ 0)])^2
  
  x2plusy2_var = 4*( hilb_mean[where(xcorr_lag EQ 0)]^2*hilb_var+ corr_mean[where(xcorr_lag EQ 0)]^2*original_var )
  corr_var = x2plusy2_var/4/(hilb_mean[where(xcorr_lag EQ 0)]^2+ corr_mean[where(xcorr_lag EQ 0)]^2)
  
  corr_value2[1] = envelope02[where(xcorr_lag EQ 0)]
  corr_std[1] = sqrt(corr_var)
endelse
...
;mean((piece-mean(piece)).*(piece02-mean(piece02)))/std(piece,1)/std(piece02,1)
  ...
  result = CREATE_STRUCT(result, 'lag', xcorr_lag*dt, 'xcorr', correlation12, 'xcorr_mean', mean_xcorr02, 'envel_mean', mean_envelope02, 'corr_value', corr_value2,'corr_std',corr_std,'subwindow_number',subwindow_number, 'ch_var', ch_var)
  return, result
end
;
