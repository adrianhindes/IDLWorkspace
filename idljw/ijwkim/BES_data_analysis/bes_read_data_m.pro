...
FUNCTION bes_read_data_m, shot, ch, trange=trange, freq_filter=freq_filter, filter_radiation_pulses=filter_radiation_pulses, fname=fname, $
...
;     filter_radiation_pulses: scalar <floating>                                                      *
;         if not 0.0, then radiation pulses are removed by invoking bes_filter_radiation.pro          *
;         typical value is 0.02, but default is 0.0.                                                                      *
...
;        NOTE: default read location is fusino01                                                      *
;******************************************************************************************************
; Return value                                                                                        *
...
; set datapath and data source
  if fusion01 eq 1 then begin
    datapath = '/BES_DATA/APDCAM/'
  endif else begin
    datapath = '/media/DATA/APDCAM/'
  endelse 

  data_source = 32 ;for KSTAR
  default, filter_radiation_pulses, 0.0
  ...
  ; get the extension
    ...
    case extension of
      '.h5' : begin ;save the data in *.hdf5 format
        sdata = CREATE_STRUCT('channel', result.channel, 'tvector', [ result.tvector[0], result.tvector[size(result.tvector,/dimensions)-1] ], 'data', result.data)
;         sdata = CREATE_STRUCT('channel', result.channel, 'data', result.data)
