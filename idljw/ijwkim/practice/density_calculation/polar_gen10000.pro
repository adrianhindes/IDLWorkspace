    ...
    for i=0L, size_r-1 do begin
      for j=0L, size_phi-1 do begin
        del_n[i, j, *] = del_n[i, j, *] + n0[k] * $
          ...
          exp(-(r_axis[i]*sin(phi_axis[j])+(phi_dot*r_axis[i]*(t_axis-t0[k])-r0[k]*sin(phi0[k])))^2.0/(2.0*lamb_y^2.0)) * $
          exp(-(t_axis-t0[k])^2.0/(2.0*tau_life^2)) * $
          cos(2.0*!pi*(r_axis[i]*sin(phi_axis[j])+(phi_dot*r_axis[i]*(t_axis-t0[k])-r0[k]*sin(phi0[k])))/lamb_y)
      endfor
    endfor
    PRINT, k
  endfor
