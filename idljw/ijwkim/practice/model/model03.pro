...
;  stop
  ...
  part_num = Long(npts_model / part_pts)
  print, part_pts
  print, part_num
  print,part_pts*part_num
 ; stop
  
  piece = fltarr(part_pts,part_num)
  
  for i = 0, part_num-1 do begin
    piece[*,i] = model[i*part_pts:(i+1)*part_pts-1]
  endfor
  
;  plot, piece[*,10]
  
  piece_f = complexarr(part_pts,part_num)
  for i = 0, part_num-1 do begin
    piece_f[*,i] = fft(piece[*,i])
  endfor

  piece_power = complexarr(part_pts,part_num)
  for i = 0, part_num-1 do begin
...
;  stop
  ...
  average_power = average_power/part_num
...
;  power_average = abs((average_f)*(conj(average_f)))
  ...
  !P.MULTI = [0,1,2]
  plot, t_gen1, model, xrange = [0, 10]
  plot, freq_total, shift(power, -N21_total), xrange = [-10, 10] ,/ylog
