pro sigproc_tde,signal_name_in_1,signal_name_in_2,td_signal_out=td_signal_out,power_signal_out=power_signal_out,$
...
; High frequency filtering (integration)
 if (keyword_set(inttime)) then begin
  signal1 = integ(signal1,inttime*1e-6/sampletime)
  signal2 = integ(signal2,inttime*1e-6/sampletime)
endif

; Bandpass filter
signal1 = bandpass_filter_data(signal1,sampletime=sampletime,$
...
signal2 = bandpass_filter_data(signal2,sampletime=sampletime,$
           filter_low=filter_low,filter_high=filter_high,filter_order=filter_order,$
           filter_symmetric=filter_symmetric,errormess=errormess)
if (errormess ne '') then begin
  if (keyword_set(errorproc)) then begin
...
if (keyword_set(cc)) then begin
  ...
  for tau=taurange_sample[0],taurange_sample[1] do begin
    global_corr_array[k] = total(signal1_cut * signal2_cut[ind+tau-taurange_sample[0]])
    k += 1
  endfor
...
if (keyword_set(cp)) then begin
  ...
  fluc_correlation,0, refch='cache/'+signal_name_in_1,plotch='cache/'+signal_name_in_2,taurange=[-30,30],timerange=timerange,$
