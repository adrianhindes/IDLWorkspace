...
;
; NOTE:
;   This routine needs to be adapted to the local installation on line 117.
;   It will not work without the 'httpget' utility by Jon Krom, based on the TWUClib.
;
; PURPOSE:
...
;*********************************************************************************************
; CHANGE THE FOLLOWING LINE TO POINT TO THE httpget CLIENT ON YOUR SYSTEM:
    ;client = 'httpget'                  ; if you can simply call httpget from the command line
    ;client = 'C:\TWU\TWUClib\httpget'                          ; for typical Windows system
    ; client = '/home/kalvin/textor/TWUClib/Standard-C-clients/httpget/httpget'    ; for typical Unix system
; This part is for fluc_proc
; We use wget for http access, therefore teh command line uses some arguments
; Comment the cmd
  info = routine_info('twuget',/source)
  twuget_path = info.path
  twuget_path = strmid(twuget_path,0,strlen(twuget_path)-strlen('twuget.pro')-1)
...
pro TWUget, W, X, Y, Z, pulsenumber=pulsenumber, signalname=isignalname, $
