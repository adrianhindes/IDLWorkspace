...
FUNCTION READ_BMP_32, File, Red, Green, Blue, Ihdr, RGB=rgb
;+
; NAME:
;	READ_BMP
;
...
nx = abs(ihdr.biwidth)		;Columns
ny = abs(ihdr.biheight)		;Rows
...
32: begin                    ;32 bits / pixel....
    a = bytarr(4, nx, ny, /nozero)
    if ((4 * nx) and 4) eq 0 then readu, unit, a $  ;Again, dunk it.
    else begin
        pad = bytarr(4 - ((4 * nx) and 4))
