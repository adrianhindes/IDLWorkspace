pro show_ecei_bes_correlation_map,shot,time,silent=silent,rrange=rrange,zrange=zrange,isotropic=isotropic,$
       ...
       refchannel=refchannel, readparams=readparams, harmonic=harmonic
;******************************************************************************
...
default,isotropic,1
default,nlevels,30
default,rrange,[1.1,3.0]
default,zrange,[-0.3,0.3]
default, readparams, 1
default, harmonic, 2

if not keyword_set(readparams) then begin
  ...
  default, hfs_freq_0, 90.25
  default, lfs_freq_0, 83.00
endif else begin
  datapath=local_default('datapath')
  fid=h5f_open(dir_f_name(datapath,dir_f_name(strtrim(shot,2),'ecei-kstar-lfs.'+string(shot, format='(I8.8)')+'.hdf5')))
  gid=h5g_open(fid, '/ECEI')
  aid=h5a_open_name(gid, 'LoFreq')
  lfs_freq_0=(h5a_read(aid))[0]
  aid=h5a_open_name(gid, 'ZoomCodeV')
  zoom1=(h5a_read(aid))[0]
  
  fid=h5f_open(dir_f_name(datapath,dir_f_name(strtrim(shot,2),'ecei-kstar-hfs.'+string(shot, format='(I8.8)')+'.hdf5')))
  gid=h5g_open(fid, '/ECEI')
  aid=h5a_open_name(gid, 'LoFreq')
  hfs_freq_0=(h5a_read(aid))[0]
  aid=h5a_open_name(gid, 'ZoomCodeV')
  zoom2=(h5a_read(aid))[0]
  
  zoom=[zoom1,zoom2]
  
endelse

default,hfs_freq,[2.5, 3.4, 4.3, 5.2, 6.1, 7.1, 8.0, 8.9] + hfs_freq_0      ;[GHz]
default,lfs_freq,[2.5, 3.4, 4.3, 5.2, 6.1, 7.1, 8.0, 8.9] + lfs_freq_0
stop
  device, decomposed=0
...
fc = 1.6e-19*bfield0/9.1e-31/2/!pi*harmonic/1e9

hfs_r = reverse(interpol(flux.r,fc,hfs_freq))
hfs_r1 = hfs_r
hfs_r2 = hfs_r
...
lfs_r = reverse(interpol(flux.r,fc,lfs_freq))
lfs_r1 = lfs_r
lfs_r2 = lfs_r
