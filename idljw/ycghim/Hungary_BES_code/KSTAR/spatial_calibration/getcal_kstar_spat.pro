function getcal_kstar_spat, shot, xyz=xyz,transpose=transpose, $
         errormess=error, prompt=prompt, old=old, area=area, detcorn=detcorn,$
         reverse=reverse
  ...
  ;              [4,8,*] variables                               *
  ;       /xyz: returns the coordinates in xyz                   *
  ;       /old: use the olde method for reading the calibration  *
  ;     ONLY WORKS IF OLD CALLED:                                *
...
if not keyword_set(old) then begin
  ...
  filename=dir_f_name(dir_f_name(datapath,strtrim(shot,2)),strtrim(shot,2)+'.spat.sav')
  if file_test(filename) then begin
    ...
    if (keyword_set(reverse)) then position=position[[3,2,1,0],*,*]
    ...
    print, 'No calibration file is present for shot '+strtrim(shot,2)+'!'
    position=-1
  endelse
endif else begin
  error = ''
