pro calibrate_kstar_spatial_2014, start_shot=start_shot
...
for shot=shotrange[0],shotrange[1] do begin
   filename=dir_f_name(datapath,dir_f_name(strtrim(shot,2),strtrim(shot,2)+'_CMOS_data_calib.sav'))
   config_filename=dir_f_name(datapath,dir_f_name(strtrim(shot,2),strtrim(shot,2)+'_config.xml'))
   if file_test(filename) or file_test(config_filename) then begin

      load_config_parameter, shot, 'Optics', 'RadialMirrorPosition', output_struct=radpos, errormess=e2
      radpos=radpos.value

      load_config_parameter, shot, 'Optics', 'VerticalMirrorposition', output_struct=vertpos, errormess=e2
      vertpos=vertpos.value

      load_config_parameter, shot, 'Optics', 'APDCAMrotationposition', output_struct=apdpos, errormess=e2
      apdpos=apdpos.value

      load_config_parameter, shot, 'Optics', 'APDCAMfiltertype', output_struct=li_filter, errormess=e2
      ...
      print, 'Shot '+strtrim(shot,2)+' is missing!'
   endelse
endfor
