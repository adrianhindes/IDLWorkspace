...
pro collect_data,ADC_mult=ADC_mult,ADC_div=ADC_div,samplediv=samplediv,meastime=meastime,$
    bits=bits,trigger=trigger,samplenumber_out=samplecount,channel_masks=channel_masks
@apdcam_common.pro
  ...
  if (trigger ge 0) then begin
    ; For triggered operation
    printf,unit,'Trigger 1 0 0 '+i2str(trigger)+' TriggerDesc.txt'
    printf,unit,'Start'
...
@apdcam_common.pro
  ...
  if (event.ID eq kstar_start_widg) then begin
    ...
    collect_data,ADC_mult=ADC_mult,ADC_div=ADC_div,samplediv=samplediv,meastime=meastime,trigger=trigger,$
      samplenumber_out=samplenumber,bits=bits,channel_masks=channel_masks
  endif
  ; Creating shot dir
...
; Writing config file
  ...
  write_shot_config_kstar,shotnumber,ADC_mult=ADC_mult,ADC_div=ADC_div,samplediv=samplediv,samplenumber=samplenumber,$
    bits=bits,trigger=trigger,channel_masks=channel_masks,mirror_pos=mirror_pos,camera_select=camera_select,datapath=dir_f_name('data',shotdir_new)
  ...
  sclk = adclk/samplediv
  sampletime = 1./(sclk*1e6)
  load_kstar_apd_data,shotnumber,sampletime=sampletime,samplecount=samplenumber,errormess=errormess
  window,0
  for i=1,32 do begin
    show_rawsignal,shotnumber,'cache/ADC'+i2str(i),int=1000,
end
...
kstar_widg=widget_base(title='KSTAR APDCAM control',xoff=0,yoff=600,xsize=200,event_pro='kstar_event',col=1,resource_name='kstar_control')
kstar_shot_widg = cw_field(kstar_widg,title='Shot:',value='0000',xsize=5,/integer,/return_events)
kstar_start_widg = widget_button(kstar_widg,value='Start measurement')
...
endelse
xmanager,'apdcam',apd_widg,event_handler='apd_event'
widget_control,apd_offs_widg,map=0
