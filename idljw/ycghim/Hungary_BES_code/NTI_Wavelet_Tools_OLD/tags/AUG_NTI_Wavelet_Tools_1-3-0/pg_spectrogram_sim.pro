...
Nyquist_freq=1/(timeax[n_elements(timeax)-1]-timeax[0])*n_elements(timeax)/2. ; Nyquist frequency (in Hz)
freqax_length=freqres/2+1 ; Positive half-axis
df=Nyquist_freq/float(freqax_length) ; Frequency resolution in Hz
stft=stft/sqrt(df) ; Normalize STFT

; Compute axis
freqax=findgen(freqax_length)/(freqax_length-1)*Nyquist_freq/1000. ; Frequency axis in kHz
