...
;========
  ...
  ;VISUALIZATION BLOCK:
	...
	res = dialog_message($
	  ...
	  '  - Frequency Range: Frequency range for the visualization can be set here.', $
...
;========================
  ...
  if (event.ID eq nti_wavelet_gui_setup_dataman_load_widg) then begin
	...
	if (data_nd le 1) then begin
	  nti_wavelet_gui_addmessage, addtext='At least two channels needed!'
  ...
  if (event.ID eq nti_wavelet_gui_setup_dataman_save_widg) then begin
    ...
    guiblock.savefile_path = dialog_pickfile(dialog_parent=event.top, /fix_filter, filter=['*.sav'], /overwrite_prompt, /write, path="./save_data",$
      ...
      strcompress(string(datablock.data_history), /remove_all)+".sav", default_extension = "sav")
...
;================
  ...
  if (event.ID eq nti_wavelet_gui_process_loadbutton_widg) then begin
	...
	if not nti_wavelet_defined(*SAVED_DATABLOCK.PHI, /nullarray) then begin	;PHI
	  nti_wavelet_gui_addmessage, addtext='ERROR --- PHI variable is undefined!'
	  load_prdata_error = 1
	endif
	if not nti_wavelet_defined(SAVED_DATABLOCK.SHOTNUMBER, /nullarray) then begin	;SHOTNUMBER
	  nti_wavelet_gui_addmessage, addtext='ERROR --- SHOTNUMBER variable is undefined!'
	  load_prdata_error = 1
	endif
	if not nti_wavelet_defined(*SAVED_DATABLOCK.THETA, /nullarray) then begin	;THETA
	  nti_wavelet_gui_addmessage, addtext='ERROR --- CHANNELS variable is undefined!'
	  load_prdata_error = 1
	endif
...
nti_wavelet_main,$
  ;INPUT:
    ...
    mode_selection=datablock.proc_mode_selection, mode_type=datablock.proc_mode_type,$
...
endif	;if (not guiblock.pre_events) then begin
