...
default,coh_avg,0
default,coh_min,0
default,transfer_selection,0
default,transfer_cohlimit,0
default,transfer_powlimit,0
default,transfer_cscale,0.4
...
;Plot 1st direction:
  ...
  if transfer_powlimit NE 0. then begin
    ...
    printdatas=shot_printdatas+$
      ...
      transf_printdatas+$
      '!Caverages: '+i2str(coh_avr)+$
      '!CCoherence limit: '+i2str(100*transfer_cohlimit)+' %'+$
      '!CPower limit: '+i2str(100*transfer_powlimit)+' %'
    if cwt_selection then begin
     title='Transfer function of '+expname+' '+i2str(shotnumber)+'-'+channelpairs_used(0,i)+'--'+channelpairs_used(1,i)
      ...
      if linear_freqax then begin
        ...
        pg_plot4,reverse(plotted,2),xrange=[min(timeax),max(timeax)],yrange=[min(freqax),max(freqax)]$
          ,/ylog,y2range=[min(-pg_log2(scaleax)),max(-pg_log2(scaleax))]$
          ,xtitle='Time (s)',ytitle='Frequency (kHz)',y2title='-log base 2 of scale'$
          ,title=title,opt=transfer_cscale,data=printdatas
      endelse
    endif else begin
     title='Transfer function of '+expname+' '+i2str(shotnumber)+'-'+channelpairs_used(0,i)+'--'+channelpairs_used(1,i)
     device,filename=pg_filename(title,dir=savepath)
     pg_plot4,plotted,xrange=[min(timeax),max(timeax)],yrange=[min(freqax),max(freqax)], $
     xtitle='Time (s)',ytitle='Frequency (kHz)',title=title,opt=transfer_cscale,data=printdatas
    endelse
  endfor
...
;Plot 2nd direction:
  ...
  if transfer_powlimit NE 0. then begin
    ...
    printdatas=shot_printdatas+$
      ...
      '!CPower limit: '+i2str(100*transfer_powlimit)+' %'
    if cwt_selection then begin
     title='Transfer function of '+expname+' '+i2str(shotnumber)+'-'+channelpairs_used(1,i)+'--'+channelpairs_used(0,i)
      device,filename=pg_filename(title,dir=savepath)
      if linear_freqax then begin
        colors_scale_exponent=1/transfer_cscale
        levels=findgen(60)^colors_scale_exponent
