...
memory=0
if datablock.proc_transf_selection OR defined(*datablock.transforms, /nullarray) then $
  memory=memory+transform_memory*channel_num
if datablock.proc_crosstr_selection OR defined(*datablock.crosstransforms, /nullarray) then $
  memory=memory+transform_memory*datablock.channelpairs_select_num
if datablock.proc_coh_selection OR defined(*datablock.coherences, /nullarray) then $
  memory=memory+transform_memory*(datablock.channelpairs_select_num*1.5+channel_num)
if (datablock.proc_coh_selection AND datablock.proc_transfer_selection) OR defined(*datablock.transfers, /nullarray) then $
  memory=memory+2*transform_memory*datablock.channelpairs_select_num
if datablock.proc_mode_selection OR defined(*datablock.modenumbers, /nullarray) then $
