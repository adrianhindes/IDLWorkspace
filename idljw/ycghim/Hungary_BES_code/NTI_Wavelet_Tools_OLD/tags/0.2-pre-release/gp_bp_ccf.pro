...
datas='Ch: '+channel1+$
      '!CWin: '+i2str(windowsize)+' ('+pg_num2str(winlength,length=4)+'ms), Mask: '+i2str(masksize)+', '+'f_max: '+pg_num2str(fmax)+$
      '!CFqs: '+i2str(bands(0))+'-'+i2str(bands(1))+' & '+i2str(bands(2))+'-'+i2str(bands(3))+' (kHz), Length: '+pg_num2str(dt,length=5)+'s'+$
      '!CAverages: '+i2str(size)+', f_d: '+pg_num2str(f_le)+'kHz, ccf_dt: '+pg_num2str(t_c[1]-t_c[0],length=5)+'ms '+'N: '+i2str(n_elements(c[*,0]))

;ccf_title=ID+': ccf-average (#'+i2str(shot)+', '+pg_num2str(trange(0),length=4)+'s-'+pg_num2str(trange(1),length=4)+'s)'
