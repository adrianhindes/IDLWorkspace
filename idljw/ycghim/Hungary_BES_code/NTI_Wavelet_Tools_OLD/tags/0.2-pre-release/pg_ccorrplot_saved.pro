...
for i=exp[0],exp[1] do begin
	...
	; Progress indicator
	print,'Calculating correlations for shot '+i2str(shot)+' with respect to channel '+i2str(j)

	; Set multiplot for correlation plotting
	if ccorrplot then begin
		!P.MULTI=[0,2,8,0,1]
		!P.CHARSIZE=1.4
		title='CCFs '+i2str(shot)+bstr
		...
		for k=1,16 do begin
			...
			if ccorrplot then begin
				...
				title='CCF '+i2str(shot)+' channels: '+i2str(j)+'-'+i2str(k)+bstr
	...
	if ccorrmaxplot then begin
		title='Maximum crosscorrelations for shot '+i2str(shot)
...
;		otv,ccorrmax[i-exp[0],*,*]*255,interp=interp
;
...
if ccorravrplot then begin
	title='Average maximum crosscorrelations for experiment '+i2str(exp[0])+'-'+i2str(exp[1])
	data='exp:!C '+i2str(exp[0])+' - '+i2str(exp[1])+$
		'!Ctrange:!C '+pg_num2str(timeax[0],length=4)+' s - '+$
			pg_num2str(timeax[n_elements(timeax)-1],length=4)+' s'+$
		'!Cwin.: '+windowname+$
		'!Cwinsize: '+i2str(windowsize)+$
