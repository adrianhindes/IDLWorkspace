...
;			!P.CHARSIZE=1.4
;			title='Correlations '+i2str(i)+' '+channel
...
;			if NOT systerr then title=title+' (error estimation based on '+i2str(nsegment)+' segments)'
			if print then device,filename=pg_filename(title) else window,/free
...
;		if NOT systerr then title=title+' (error estimation based on '+i2str(nsegment*expsize)+' segments)'
		if print then device,filename=pg_filename(title) else window,/free
