...
for i=exp[0],exp[1] do begin
	for j=1,channelssize do begin
		...
		if aplot then begin
			...
			!P.CHARSIZE=1.4
			ftitle='Autocorrelations '+i2str(i)+' '+channel
		endif
		...
		for k=0,filenamessize-1 do begin
			...
			if aplot then begin
				if k/4. EQ k/4 then begin
					if print then device,filename=pg_filename(ftitle+'-'+i2str(k/4)) else window,/free
					xyouts,0.1,0.18,data,/normal
				endif

				title='Autocorrelation of band power'+str[k]
				title=title+' (error estimation based on '+i2str(nsegment)+' segments)'
		...
		if cplot then begin
			!P.MULTI=[0,1,(filenamessize+1) < 5,0,1]
			!P.CHARSIZE=1.4
			ftitle='Crosscorrelations '+i2str(i)+' '+channel
		endif
		...
		for k=0,filenamessize-2 do begin
			for m=k+1,filenamessize-1 do begin
				...
				if cplot then begin
					...
					if print then device,filename=pg_filename(ftitle+'-'+i2str(c/4)) else window,/free
						xyouts,0.1,0.18,data,/normal
					endif
	...
	if shotplot then begin
		for j=0,filenamessize-1 do begin
			title='Average ACF '+str[j]+' for shot '+i2str(i)
			if err EQ 0 then title=title+' (error estimation based on '+i2str(nsegment*channelssize)+' segments)'
			if err EQ 1 then title=title+' (error estimation based on '+i2str(nsegment)+' segments)'
			if err EQ 2 then title=title+' (error estimation based on '+i2str(channelssize)+' segments)'
			if err EQ 3 then title=title+' (error estimation based on '+i2str(nsegment*channelssize)+' segments)'
		...
		for j=0,(filenamessize^2-filenamessize)/2-1 do begin
			title='Average CCF '+str2[j]+' for shot '+i2str(i)
			if err EQ 0 then title=title+' (error estimation based on '+i2str(nsegment*channelssize)+' segments)'
			if err EQ 1 then title=title+' (error estimation based on '+i2str(nsegment)+' segments)'
			if err EQ 2 then title=title+' (error estimation based on '+i2str(channelssize)+' segments)'
			if err EQ 3 then title=title+' (error estimation based on '+i2str(nsegment*channelssize)+' segments)'
...
if expplot then begin
	data='experiment: '+i2str(exp[0])+'-'+i2str(exp[1])+$
		'!Ctrange:!C '+pg_num2str(timeax[0],length=4)+' s - '+$
			pg_num2str(timeax[n_elements(timeax)-1],length=4)+' s'+$
		'!Cwin.: '+windowname+$
		'!Cwinsize: '+i2str(windowsize)+$
	...
	for j=0,filenamessize-1 do begin
		title='Average ACF '+str[j]+' for the experiment ('+i2str(exp[0])+'-'+i2str(exp[1])+')'
		if err EQ 0 then title=title+' (error estimation based on '+i2str(nsegment*channelssize*expsize)+' segments)'
		if err EQ 1 then title=title+' (error estimation based on '+i2str(nsegment*expsize)+' segments)'
		if err EQ 2 then title=title+' (error estimation based on '+i2str(expsize)+' segments)'
		if print then device,filename=pg_filename(title) else window,/free
	...
	for j=0,(filenamessize^2-filenamessize)/2-1 do begin
		title='Average CCF '+str2[j]+' for the experiment ('+i2str(exp[0])+'-'+i2str(exp[1])+')'
		if err EQ 0 then title=title+' (error estimation based on '+i2str(nsegment*channelssize*expsize)+' segments)'
		if err EQ 1 then title=title+' (error estimation based on '+i2str(nsegment*expsize)+' segments)'
		if err EQ 2 then title=title+' (error estimation based on '+i2str(expsize)+' segments)'
		if print then device,filename=pg_filename(title) else window,/free
...
for i=exp[0],exp[1] do begin
	for j=1,channelssize do begin
		...
		data='shot: '+i2str(i)+' '+channel+$
			'             trange: '+pg_num2str(timeax[0],length=4)+' s - '+$
				pg_num2str(timeax[n_elements(timeax)-1],length=4)+' s'+$
			'!Cwin.: '+windowname+$
			'             winsize: '+i2str(windowsize)+$
			'             masksize: '+i2str(masksize)+$
			'!Cfreqres: '+i2str(freqres)+$
			'             step: '+i2str(step)+$
			'!Cblocksize: '+i2str(blocksize)
		...
		if cplot then begin
			!P.MULTI=[0,1,3,0,1]
			!P.CHARSIZE=1.4
			ftitle='Phase coherence '+i2str(i)+' '+channel
		endif
		if aplot then begin
			!P.MULTI=[0,1,6,0,1]
			!P.CHARSIZE=1.4
			ftitle='Combined coherence '+i2str(i)+' '+channel
		endif
		...
		for k=0,filenamessize-2 do begin
			for m=k+1,filenamessize-1 do begin
				...
				if cplot then begin
					if print then device,filename=pg_filename(ftitle+'-'+str[k]+' and'+str[m],dir=i2str(exp[0])+'-'+i2str(exp[1])) else window,/free
				endif
...
;					errphase=1/!PI/(1+temp_cohphase[*,1]^2)*temp_cohphase[*,1]*sqrt((Imaginary(errcpsd)/Imaginary(meancpsd))^2+(Float(errcpsd)/Float(meancpsd))^2) ;Wrong
	...
	if shotplot then begin
		!P.MULTI=[0,1,6,0,1]
		!P.CHARSIZE=1.4
		ftitle='Combined coherence for shot: '+i2str(i)
	endif
		data='shot: '+i2str(i)+$
			'             trange: '+pg_num2str(timeax[0],length=4)+' s - '+$
				pg_num2str(timeax[n_elements(timeax)-1],length=4)+' s'+$
...
if expplot then begin
	...
	!P.MULTI=[0,1,6,0,1]
	!P.CHARSIZE=1.4
	ftitle='Combined coherence for experiment: '+i2str(exp[0])+'-'+i2str(exp[1])
endif
