...
;	 derived from data in the Nicolet system (mode advanced and demanding methods,
;	 than pg_modenumber)
;
; Calling sequence:
;	modenum=pg_modenumber_full(shot [,channels] [,chpos] [,trange] [,filters] [,windowname] [,windowsize] [,masksize] [,step] $
;		[,freqres] [,fmax] [,/cwt] [,family] [,order] [,dscale] [,start_scale] [,nscale] [,modescale] [,fmax]$
;		[,plot] [,avr] [,/print] [,opt] [,filterparam1] [,filterparam2] [,filterparam3] [,cohlim] [,powlim]$
...
print,'Modenumbers for '+i2str(shot)
wait,0.1
...
for i=0,smoothed_transsize(1)-1 do begin
	for j=0,smoothed_transsize(2)-1 do begin
		...
		; Filter 0: positive-negative modenumbers from relative positions
    ...
    ; Filter 1: positive-negative modenumbers from relative positions
    ; (linear fit with weighting COH/median(COH))
    ...
    ; Filter 2: positive-negative modenumbers from relative positions
...
if modenumberplot then begin
	...
	if cwt then begin
	  title='Modenumbers of shot: '+i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
		if print then device,filename=pg_filename(title) else window,/free
		pg_plot4,reverse(modenum,2),ct=42,xrange=[min(timeax),max(timeax)],yrange=[min(freqax),max(freqax)] $
			...
			;,/original
    if transplot EQ 3 then begin
      title='Linear axis modenumbers of shot: '+i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
      if print then device,filename=pg_filename(title) else window,/free
      loadct,42
    ...
    if transplot GE 4 then begin
      title='Linear 0 axis modenumbers of shot: '+i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
      if print then device,filename=pg_filename(title) else window,/free
      loadct,42
      levels=findgen(fix(modescale(1)-modescale(0)+1))
      plotted=modenum
	...
	if (avr GT 0) and keyword_set(cohlim) then begin
		...
		; Plot
		if cwt then begin
      title='Modenumbers (cohlim) of shot: '+i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
			if print then device,filename=pg_filename(title) else window,/free
			pg_plot4,reverse(modenum,2),ct=42,xrange=[min(timeax),max(timeax)],yrange=[min(freqax),max(freqax)] $
				,/ylog,y2range=[min(-pg_log2(scaleax)),max(-pg_log2(scaleax))] $
				,xtitle='Time (s)',ytitle='Frequency (kHz)',y2title='-log base 2 of scale' $
				...
				;,/original
      if transplot EQ 3 then begin
        title='Linear axis modenumbers (cohlim) of shot: '+i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
        if print then device,filename=pg_filename(title) else window,/free
        loadct,42
        levels=findgen(fix(modescale(1)-modescale(0)+1))
        plotted=modenum
      ...
      if transplot GE 4 then begin
        title='Linear 0 axis modenumbers (cohlim) of shot: '+i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
        if print then device,filename=pg_filename(title) else window,/free
        loadct,42
        levels=findgen(fix(modescale(1)-modescale(0)+1))
        plotted=modenum
	...
	if keyword_set(powlim) then begin
		...
		; Plot
		if cwt then begin
      title='Modenumbers (powlim+) of shot: '+i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
			if print then device,filename=pg_filename(title) else window,/free
			pg_plot4,reverse(modenum,2),ct=42,xrange=[min(timeax),max(timeax)],yrange=[min(freqax),max(freqax)] $
				,/ylog,y2range=[min(-pg_log2(scaleax)),max(-pg_log2(scaleax))] $
				,xtitle='Time (s)',ytitle='Frequency (kHz)',y2title='-log base 2 of scale' $
				...
				;,/original
      if transplot EQ 3 then begin
        title='Linear axis modenumbers (powlim+) of shot: '+i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
        if print then device,filename=pg_filename(title) else window,/free
        loadct,42
        levels=findgen(fix(modescale(1)-modescale(0)+1))
        plotted=modenum
      ...
      if transplot GE 4 then begin
        title='Linear 0 axis modenumbers (powlim+) of shot: '+i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
        if print then device,filename=pg_filename(title) else window,/free
        loadct,42
        levels=findgen(fix(modescale(1)-modescale(0)+1))
        plotted=modenum
  ...
  if (where(filters EQ 1) GT -1) OR (where(filters EQ 2) GT -1) then begin
    ...
    for i=0,n_elements(filterparam2)-1 do begin
      ...
      ; Plot
      if cwt then begin
        title='Modenumbers (global Q: '+i2str(filterparam2(i))+') of shot: '$
        ...
        pg_plot4,reverse(modenum,2),ct=42,xrange=[min(timeax),max(timeax)],yrange=[min(freqax),max(freqax)] $
          ...
          ;,/original
        if transplot EQ 3 then begin
          title='Linear axis modenumbers (global Q: '+i2str(filterparam2(i))+') of shot: '$
            +i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
          if print then device,filename=pg_filename(title) else window,/free
          loadct,42
          levels=findgen(fix(modescale(1)-modescale(0)+1))
        ...
        if transplot GE 4 then begin
          title='Linear 0 axis modenumbers (global Q: '+i2str(filterparam2(i))+') of shot: '$
            +i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
          if print then device,filename=pg_filename(title) else window,/free
          loadct,42
          levels=findgen(fix(modescale(1)-modescale(0)+1))
  ...
  if (where(filters EQ 1) GT -1) OR (where(filters EQ 2) GT -1)then begin
    ...
    for i=0,n_elements(filterparam2)-1 do begin
      ...
      ; Plot
      if cwt then begin
        title='Modenumbers (cohlim + global Q: '+i2str(filterparam2(i))+') of shot: '$
          +i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
        if print then device,filename=pg_filename(title) else window,/free
        pg_plot4,reverse(modenum,2),ct=42,xrange=[min(timeax),max(timeax)],yrange=[min(freqax),max(freqax)] $
          ,/ylog,y2range=[min(-pg_log2(scaleax)),max(-pg_log2(scaleax))] $
          ...
          ;,/original
        if transplot EQ 3 then begin
          title='Linear axis modenumbers (cohlim + global Q: '+i2str(filterparam2(i))+') of shot: '$
            +i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
          if print then device,filename=pg_filename(title) else window,/free
          loadct,42
          levels=findgen(fix(modescale(1)-modescale(0)+1))
        ...
        if transplot GE 4 then begin
          title='Linear 0 axis modenumbers (cohlim + global Q: '+i2str(filterparam2(i))+') of shot: '$
            +i2str(shot)+' '+channels(0)+' '+channels(1)+'...'
          if print then device,filename=pg_filename(title) else window,/free
          loadct,42
          levels=findgen(fix(modescale(1)-modescale(0)+1))
