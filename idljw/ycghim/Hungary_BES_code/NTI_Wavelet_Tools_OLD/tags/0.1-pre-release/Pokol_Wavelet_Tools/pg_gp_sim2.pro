...
default,n,1000
default,m,300
default,perturb_pos,3.
default,sigma,0.5
default,c,1
default,modenumber,3
default,reffon,1
default,realgeometry,1
default,plot,0
...
filename='fluxsurfshot_'+i2str(shot)+'_time_'+pg_num2str(time,length=4)+'_phi_'+pg_num2str(PHIsurf, length=4)+$
	'_beta_0.000'+'.dat'
...
if plot then begin
plot,raxis_interpolated,zaxis_interpolated,PSYM=3,title='interpolated fluxsurf in shot '+i2str(shot)+', n='+i2str(n)+' m='+i2str(m), xrange=[160,240],yrange=[-50,50]
...
for i=0,15 do begin
	fn='data/'+i2str(number)+'_gp_sim_channel_'+i2str(i+1)+'.sav'
	data=reform(mir[i,*])
	sampletime=dt
	starttime=trange(0)
	endtime=trange(1)
