...
; Rewritten by: Gergo Pokol (pokol@reak.bme.hu) 2004.12.08. (frequency axis added)
;
...
if keyword_set(fmax) then begin
   ...
   timeax=float(dindgen(n_elements(data))*(double(timeax(n_elements(timeax)-1))-double(timeax(0)))/(n_elements(data)-1))+timeax(0)
...
freqax=order/scale/(timeax[1]-timeax[0])/1000/2/!PI
...
if keyword_set(ID) then data='ID='+ID+'!C'+data
...
if (plot EQ 0) OR (plot EQ 1) OR (plot EQ 3) then begin ; Scalogram (log)
	...
	pg_plot4,plotted,ct=5,xrange=[min(timeax),max(timeax)],yrange=[min(freqax),max(freqax)]$
		...
		,title=title,zlog=log,opt=opt,data=data, poster=poster

endif
if (plot EQ 0) OR (plot EQ 2) then begin ; Phase (log)

	;Compute Phase
	plotted=reverse(atan(float(cwt),imaginary(cwt)),2)
	...
	pg_plot4,plotted,ct=0,xrange=[min(timeax),max(timeax)],yrange=[min(freqax),max(freqax)]$
		,/ylog,y2range=[min(-pg_log2(scale)),max(-pg_log2(scale))]$
		,xtitle='Time (s)',ytitle='Frequency (kHz)',y2title='-log base 2 of scale'$
		,title=title,data=data, poster=poster
