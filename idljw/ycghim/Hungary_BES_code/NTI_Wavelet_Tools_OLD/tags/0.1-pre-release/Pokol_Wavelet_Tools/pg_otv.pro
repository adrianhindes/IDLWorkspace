...
if ((!d.name eq 'X') or (!d.name eq 'WIN')) then begin
	...
	sx=px(1)-px(0)-1
	sy=py(1)-py(0)-1
	tv,congrid(q,sx,sy),px(0)+1,py(0)+1
endif
...
if (!d.name eq 'PS') then begin
	...
	if original then begin
		...
		sx=(size(q))(1)<psdim(0)
		sy=(size(q))(2)<psdim(1)
		if nearest then tv,congrid(q,sx,sy),px(0),py(0),xsize=px(1)-px(0),ysize=py(1)-py(0),/normal $
			else tv,congrid(q,sx,sy,/center,cubic=-0.5),px(0),py(0),xsize=px(1)-px(0),ysize=py(1)-py(0),/normal
	endelse
endif
