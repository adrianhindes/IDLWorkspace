; Name: pg_modefilter_varfit
;
; Written by: Gergo Pokol (pokol@reak.bme.hu) 2007.04.17.
;
; Purpose: Filter modenumbers from pre-calculated cross-phases by linear regression without
;				the condition of being strictly monotone
;				(Can be used for a set of probes limited to a segment of the cross section)
;
; Calling sequence:
;	modenum=pg_modefilter_varfit(cphases, chpos [,problim] [,chisqlim])
;
; Inputs:
...
function pg_modefilter_varfit, cphases, chpos, problim=problim, chisqlim=chisqlim
...
fit0=linfit(chpos/360.*2*!PI,meanphases,measure_errors=stddevphases,sigma=sigma0,chisq=chisq0,$
...
for i=0,channelssize-1 do begin
	for j=1,channelssize-1 do begin
		...
		if chisqopt LT chisq0 then begin
			fit0=fitopt
			phases=phasesopt
			sigma0=sigmaopt
			chisq0=chisqopt
...
stddevphases(0)=1.e-15 ; Force a=0
fit1=linfit(chpos/360.*2*!PI,meanphases,measure_errors=stddevphases,sigma=sigma1,chisq=chisq1,$
...
if fit(0,1) EQ -fit(1,1) then begin
	...
	if fit(0,1) LT -fit(1,1) then good=0 else good=1
