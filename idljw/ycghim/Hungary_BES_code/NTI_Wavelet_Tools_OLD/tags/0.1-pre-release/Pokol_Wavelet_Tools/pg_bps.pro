...
for i=exp[0],exp[1] do begin
	...
	if bpplot then begin
		...
		print,'Plotting band powers for shot '+i2str(i)



		for k=0,bsize[2]-1 do begin

			title='Band powers '+i2str(i)+' '+str[k]

			if print then device,filename=pg_filename(title) else window,/free

			for j=1,16 do begin

				channel='W7-AS Mirnov/Mir-1-'+i2str(j)

				title='Band power '+i2str(i)+' '+channel+str[k]
	...
	if splot then begin
		...
		print,'Plotting spectra for shot '+i2str(i)



		title='Spectra '+i2str(i)
		...
		for j=1,16 do begin
			...
			title='Spectrum '+i2str(i)+' '+channel
...
for k=0,bsize[2]-1 do begin
	...
	title='Band powers '+str[k]+' of shots: '+i2str(exp[0])+'-'+i2str(exp[1])
...
if cwt then filename=pg_filename('cwt'+i2str(order)+'_filenames',ext='.dat') $

else filename=pg_filename('stft'+i2str(windowsize)+'_filenames',ext='.dat')
