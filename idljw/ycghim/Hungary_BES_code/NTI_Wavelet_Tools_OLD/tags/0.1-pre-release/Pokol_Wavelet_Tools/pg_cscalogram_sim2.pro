; Name: pg_cscalogram_sim2
;
; Written by: Gergo Pokol (pokol@reak.bme.hu) 2006.11.20.
;
; Purpose: Plot the Scalograms, Phase of the Cross Wavelet Spectrum and Coherence of two 1D data vectors
;
; Calling sequence:
;	ccwt=pg_cscalogram_sim2(data1, data2, timeax [,shot] [,channel] [,trange] [,family] [,order] [,dscale] [,start_scale] [,nscale]  $
;		[,plot] [,inttime] [,phaselevel] [,/print] [,/pad] [,/log] [,opt] [,/double] [,freqax])
;
; Inputs:
;	data1: signal 1
;	data2: signal 2
...
function pg_cscalogram_sim2, data1, data2, timeax, shot=shot, channel=channel, trange=trange, family=family, order=order, dscale=dscale,$
...
datas='shot1: '+i2str(shot[0])+$
	...
	'!Cnscale: '+i2str(nscale)+$
	'!Cintegration time: '+i2str(inttime)+'!C '+pg_num2str((timeax[1]-timeax[0])*inttime)+'s'
...
if ((plot EQ 0) OR (plot EQ 1)) AND (inttime GT 2) then begin
	ploted=coh
...
if ((plot EQ 0) OR (plot EQ 3)) AND (family EQ 'Morlet') AND (inttime GT 2) then begin
...
if phaselevel NE 0 then begin
	print,'phaselevel'
	ploted[where(abs(cpsd) LT max(abs(cpsd))/100*phaselevel)]=(257/255)*!PI
	if (plot EQ 0) OR (plot EQ 1) then begin
