...
;               27 --> AUG H-alpha diagnostic (POT) /L.N. 2010.09.20.
; 
...
forward_function nti_wavelet_default

nti_wavelet_default,shot,0
nti_wavelet_default,data_source,16      ; Nicolet at W7-AS
nti_wavelet_default,datapath,'data/'
nti_wavelet_default,savedata,1
nti_wavelet_default,movedata,''

forward_function strsplit
forward_function nti_wavelet_i2str
...
if ((data_source eq 8) or (data_source eq 10)) then begin
  if ((size(signal_in))(1) ne 7) then signal_in='Blo-'+nti_wavelet_i2str(signal_in) $
            else signal_in=string(signal_in)
endif else begin
  if (((data_source le 5) or (data_source eq 7) or (data_source eq 9) or (data_source eq 11)) and $
    ((size(signal_in))(1) ne 7)) then signal_in='Li-'+nti_wavelet_i2str(signal_in) $
              else signal_in=string(signal_in)
endelse
full_signal_in = sysname+'/'+signal_in

; Handling subchannels
nti_wavelet_default,subchannel,0
...
if (data_source eq 0) then begin
  ...
  if ((subchannel ge 1) and (subchannels le 1)) then begin
    errormess='Subchannels are not available in shot '+nti_wavelet_i2str(shot)+'. Not a deflected beam measurement.'
    if (keyword_set(errorproc)) then begin
      call_procedure,errorproc,errormess,/forward
    endif else begin
      print,errormess
  ...
  if (subchannel gt subchannels) then begin
    errormess='Subchannel '+nti_wavelet_i2str(subchannel)+' is not available in shot '+nti_wavelet_i2str(shot)
    if (keyword_set(errorproc)) then begin
      call_procedure,errorproc,errormess,/forward
    endif else begin
      print,errormess
  ...
  if (subchannels le 1) then begin
    ...
    if ((strmid(signal_in,0,3) eq 'Li-') and (not keyword_set(nocalibrate))) then begin
      ...
      if ((size(calfac))(0) eq 0) then begin
        errormess='Cannot find calibration data for shot '+nti_wavelet_i2str(shot)
        if (keyword_set(errorproc)) then begin
          call_procedure,errorproc,errormess,/forward
        endif else begin
          print,errormess
    ...
    if (period_time eq 0) then begin
      errormess='Shot '+nti_wavelet_i2str(shot)+' is not deflected beam shot.'
      if (keyword_set(errorproc)) then begin
        call_procedure,errorproc,errormess,/forward
      endif else begin
        print,errormess
    ...
    nti_wavelet_default,trange,[starttime,starttime+10]
       trange=float(trange)
...
if (data_source eq 5) then begin
  ...
  if ((strmid(signal_in,0,3) eq 'Li-') and (not keyword_set(nocalibrate))) then begin
    if (not keyword_set(calfac)) then calfac=getcal(shot,data_source=data_source,/silent)
    if ((size(calfac))(0) eq 0) then begin
      errormess='Cannot find calibration data for AUG shot '+nti_wavelet_i2str(shot)
      if (keyword_set(errorproc)) then begin
        call_procedure,errorproc,errormess,/forward
      endif else begin
        print,errormess
...
openr, unit, 'data/'+nti_wavelet_i2str(shot)+'standard_li.sav', error=errli, /get_lun

  if  (errli EQ 0) then begin
    close, unit  & free_lun,unit
    restore, 'data/'+nti_wavelet_i2str(shot)+'standard_li.sav'
    ...
    if (not keyword_set(nosave)) then begin
      ...
      data_arr[0,*] = time
      save,data_arr,time,file=dir_f_name('data',nti_wavelet_i2str(shot)+'standard_li.sav')
    endif else begin
      rawchannel,shot,'PELLET','LIB3',channel,pointn,data,time
    endelse
...
if (ind(0) lt 0) then begin
  txt='Signal '+ch_str+' is not available in '+syst_names(data_source)+' in shot '+nti_wavelet_i2str(shot,digits=5)
      if (keyword_set(errorproc)) then begin
        call_procedure,errorproc,txt,/forward
      endif else begin
        print,txt
...
; Now calibrating
  if ( (sigarr[0] eq 'Li') and (not keyword_set(nocalibrate))) then begin
    if (not keyword_set(calfac)) then calfac=getcal(shot,data_source=data_source,/silent)
    if ((size(calfac))(0) eq 0) then begin
      errormess='Cannot find calibration data for W7-AS shot '+nti_wavelet_i2str(shot)
      if (keyword_set(errorproc)) then begin
        call_procedure,errorproc,errormess,/forward
      endif else begin
        print,errormess
...
if (data_source eq 3) then begin
  ...
  directory = 'data'
  file_name = directory+'/'+'AUG'+nti_wavelet_i2str(shot,digits=5) + '_time.sav'
  ...
  restore,file_name
  time=signal_time
  file_name = directory+'/'+'AUG'+nti_wavelet_i2str(shot,digits=5) + '_' +nti_wavelet_i2str(chn,digits=2)+ '.sav'
  openr,unit,file_name,error=error,/get_lun
  if (error ne 0) then begin
    txt='Cannot open data file '+file_name
    if (keyword_set(errorproc)) then begin
...
if (data_source eq 4) then begin    ; *** Mirnov data from W7-AS Mirnov system
  ...
  modname=w(0)+'-'+w(1)
  buffname=datapath+'mir_'+nti_wavelet_i2str(shot,digits=5)+'_'+strupcase(modname)+'.sav'
...
if (data_source eq 7) then begin  ; *************** TEXTOR Li-beam signal ***************
  ...
  if (ind(0) lt 0) then begin
    errormess='Signal '+signal_in+' is not available in '+syst_names(data_source)+  $
           ' in shot '+nti_wavelet_i2str(shot,digits=5)
    if (keyword_set(errorproc)) then begin
      call_procedure,errorproc,errormess,/forward
    endif else begin
      print,errormess
  ...
  if (ok eq 0) then begin
    errormess = 'Cannot read signal '+signal_in+' in shot '+nti_wavelet_i2str(shot)
    if (keyword_set(errorproc)) then begin
      call_procedure,errorproc,errormess,/forward
    endif else begin
      print,errormess
...
; the start time is OK, we use ext_fsample for sample frequency
  ...
  if (not keyword_set(nocalibrate)) then begin
    ...
    if (ind(0) lt 0) then begin
      print,'No calibration data is available for channel '+nti_wavelet_i2str(ch)
      stop
    endif
...
if (data_source eq 8) then begin  ; TEXTOR Blow-off signal
  ...
  get_rawsignal,data_names=syst_names
  if (ind(0) lt 0) then begin
    errormess='Signal '+signal_in+' is not available in '+syst_names(data_source)+' in shot '+nti_wavelet_i2str(shot,digits=5)
    if (keyword_set(errorproc)) then begin
      call_procedure,errorproc,errormess,/forward
    endif else begin
      print,errormess
...
if (data_source eq 11) then begin
  ...
  while (window_counter ge 0) do begin
    node = 'DI/C1-CATS<KG1:'+nti_wavelet_i2str(ch,digits=3)+'/'+nti_wavelet_i2str(window_counter)
    jpfget, node=node, pulno=shot, data=data_win, tvec=time_win, unit=unit,$
                      pulsefile='LPF', diag='CATS1', ier=ier
    if (ier ne 0) then begin
      window_counter = -1
    endif else begin
    print,nti_wavelet_i2str(window_counter)+'   ['+string(time_win(0),format='(F6.3)')+' , '+string(time_win(n_elements(time_win)-1),format='(F6.3)')+']'
  ...
  if (n_elements(time) le 1) then begin
    errormess='Error reading signal '+signal_in+' in '+syst_names(data_source)+' in shot '+nti_wavelet_i2str(shot,digits=5)+' (jpfget error:'+nti_wavelet_i2str(ier)+')'
    if (keyword_set(errorproc)) then begin
      call_procedure,errorproc,errormess,/forward
    endif else begin
      print,errormess
...
;  endif
  ...
  if (strmid(signal_in,0,7) eq 'Line-1-') then begin
    ch_txt = nti_wavelet_i2str(fix(strmid(signal_in,7))-1);
    fname = dir_f_name('data','ch'+ch_txt+'l1'+nti_wavelet_i2str(shot)+'.dat')
  endif
  if (strmid(signal_in,0,7) eq 'Line-2-') then begin
    ch_txt = nti_wavelet_i2str(fix(strmid(signal_in,7))-1);
    fname = dir_f_name('data','ch'+ch_txt+'l2'+nti_wavelet_i2str(shot)+'.dat')
  endif
  if (strmid(signal_in,0,5) eq 'Dens-') then begin
    ch_txt = nti_wavelet_i2str(fix(strmid(signal_in,5))-1);
    fname = dir_f_name('data','ch'+ch_txt+'dens'+nti_wavelet_i2str(shot)+'.dat')
  endif
  if (strmid(signal_in,0,5) eq 'Temp-') then begin
    ch_txt = nti_wavelet_i2str(fix(strmid(signal_in,5))-1);
    fname = dir_f_name('data','ch'+ch_txt+'temp'+nti_wavelet_i2str(shot)+'.dat')
  endif
...
if (data_source eq 13) then begin
  ...
  if (not keyword_set(filename)) then begin
    filename = 'ni'+nti_wavelet_i2str(card)+'_'+nti_wavelet_i2str(shot,digits=5)+'.dat'
  endif
...
if (data_source eq 14) then begin
  ...
  file = 'data/ecetemp'+nti_wavelet_i2str(shot,digits=5)+'.dat'
...
if (data_source eq 15) then begin
  ...
  str = '_sig=jet("ppf/kk3/te'+nti_wavelet_i2str(fix(signal_in))+'",'+nti_wavelet_i2str(shot,digits=5)+')'
  data = mdsvalue(str)
  time = mdsvalue('dim_of(_sig)')
  mdsdisconnect
  if (not keyword_set(data) or (not keyword_set(time))) then begin
    get_rawsignal,data_names=syst_names
    errormess='Cannot read '+signal_in+' in '+syst_names(data_source)+' in shot '+nti_wavelet_i2str(shot,digits=5)
    if (keyword_set(errorproc)) then begin
      call_procedure,errorproc,errormess,/forward
    endif else begin
      print,errormess
...
if (data_source eq 16) then begin
  if (keyword_set(nodata)) then return
  fname = datapath+nti_wavelet_i2str(shot,digits=5)+signal_in+'.sav'
  restore,fname
  if (not nti_wavelet_defined(data) or not nti_wavelet_defined(sampletime) or not nti_wavelet_defined(starttime)) then begin
...
if (data_source eq 19) then begin
  ...
  sig_type=''
  dfp= 0
  post_url='/textor/all/'+nti_wavelet_i2str(shot,digits=5)+'/'+sss
...
; normal channels
   get_lun, unit1
   ;openr, unit1, nti_wavelet_i2str(shot)+'/'+nti_wavelet_i2str(shot)+'hebeam.sav', error=errhe
...
ENDIF ELSE BEGIN
   ...
   get_lun, unit1
   ;openr, unit1, nti_wavelet_i2str(shot)+'/'+nti_wavelet_i2str(shot)+'tails.sav', error=errhe
...
if (data_source eq 21) then begin
  ...
  read_ni6115,datapath+nti_wavelet_i2str(shot)+signal_in,ch,config=$
...
if (data_source eq 22) then begin
   ...
   filename = dir_f_name('data','AUG_'+nti_wavelet_i2str(shot)+'_'+signal_in+'.sav')
...
if data_source eq 24 then begin
	...
	filename = nti_wavelet_i2str(shot)+'_AUG_SXR_'+signal_in+'.sav'
	dir='./data/'
...
if data_source eq 25 then begin
	...
	filename = nti_wavelet_i2str(shot)+'_AUG_ECE.sav'
	dir='./data/'
	if keyword_set(datapath) then dir=datapath
	...
	;if file does not exists then download it
		...
		print, ' '
		if error EQ 0 then begin 
		    print,'Succesfully downloaded data from shot '+nti_wavelet_i2str(shot)
		endif else begin
		    print,'Error in read_cec.pro, error code: '+nti_wavelet_i2str(error)
		endelse
...
if data_source eq 26 then begin
	...
	filename = nti_wavelet_i2str(shot)+'_AUG_FILD_'+signal_in+'.sav'
	dir='./data/'
	if keyword_set(datapath) then dir=datapath
	save_file=dir+filename
	...
	;if file does not exists then download it
		...
		if ier EQ 0 then begin 
		    ...
		    print,'Error in read_signal.pro, error code: '+nti_wavelet_i2str(ier)
		endelse
...
if data_source eq 27 then begin
  ...
  filename = nti_wavelet_i2str(shot)+'_AUG_POT_'+signal_in+'.sav'
  dir='./data/'
  if keyword_set(datapath) then dir=datapath
   save_file=dir+filename
  ...
  ;if file does not exists then download it
    ...
    read_signal, ier, shot, 'POT', signal_in, time, data, dim
