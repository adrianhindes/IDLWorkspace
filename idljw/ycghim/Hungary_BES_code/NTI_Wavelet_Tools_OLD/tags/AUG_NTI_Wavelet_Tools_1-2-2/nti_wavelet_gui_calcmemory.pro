...
memory=0
if datablock.proc_transf_selection OR nti_wavelet_defined(*datablock.transforms, /nullarray) then $
  memory=memory+transform_memory*channel_num
if datablock.proc_crosstr_selection OR nti_wavelet_defined(*datablock.crosstransforms, /nullarray) then $
  memory=memory+transform_memory*datablock.channelpairs_select_num
if datablock.proc_coh_selection OR nti_wavelet_defined(*datablock.coherences, /nullarray) then $
  memory=memory+transform_memory*(datablock.channelpairs_select_num*1.5+channel_num)
if (datablock.proc_coh_selection AND datablock.proc_transfer_selection) OR nti_wavelet_defined(*datablock.transfers, /nullarray) then $
  memory=memory+2*transform_memory*datablock.channelpairs_select_num
if datablock.proc_mode_selection OR nti_wavelet_defined(*datablock.modenumbers, /nullarray) then $
  memory=memory+transform_memory*(datablock.channelpairs_select_num+2)
