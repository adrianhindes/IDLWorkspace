...
;      Float[number of points in the time axis of the transform,number of points in the frequency axis of the transform]
;    errtransforms: contains the error on the STFT
;	errtransforms.abs.value: absolute value of the STFT
;	errtransforms.abs.error: error on the absolute value of the STFT
;	errtransforms.phase.value: phase of the STFT
;	errtransforms.phase.error: error on the phase of the STFT
;	errtransforms.ed.value: energy density caluclated from STFT
;	errtransforms.ed.error: error on the energy density
;    errcrosstransforms: contains the error on Crosstransforms in the case of STFT
;	errcrosstransforms.abs.value: absolute value of the crosstransforms
;	errcrosstransforms.abs.error: error on the absolute value of the crosstransforms
;	errcrosstransforms.phase.value: phase of the crosstransforms
;	errcrosstransforms.phase.error: error on the phase of the crosstransforms
;
;-
...
pro nti_wavelet_main,$
  ...
  ; Output
    ...
    errtransforms=errtransforms, errcrosstransforms=errcrosstransforms
...
if transf_selection then begin
  if cwt_selection then begin
	...
	if error then begin
	  ;Create errtransforms structure
	  errtransforms={abs:{value:fltarr(channelsize,transformsize(1),transformsize(2)),error:fltarr(channelsize,transformsize(1),transformsize(2))},$
	  ...
	  errtransforms.abs.error=fltarr(channelsize,transformsize(1),transformsize(2))
	  errtransforms.abs.error(0,*,*)=abs_err(*,0:transformsize(2)-1) ;Store for positive frequencies only
	  errtransforms.phase.error=fltarr(channelsize,transformsize(1),transformsize(2))
	  errtransforms.phase.error(0,*,*)=phase_err(*,0:transformsize(2)-1) ;Store for positive frequencies only
	endif
  	for i=1,channelsize-1 do begin
		...
		if error then begin
		  errtransforms.abs.error(i,*,*)=abs_err(*,0:transformsize(2)-1) ;Store for positive frequencies only
		  errtransforms.phase.error(i,*,*)=phase_err(*,0:transformsize(2)-1) ;Store for positive frequencies only
		endif
	...
	;Calculate errtransforms
	  if error then begin
	    ;Absolute value
	    errtransforms.abs.value=abs(transforms)
	    ;Phase
	    errtransforms.phase.value=atan(transforms, /phase)
	    ;Energy density
	    errtransforms.ed.value=2*(abs(transforms)^2)
	    errtransforms.ed.error=4*abs(transforms)*errtransforms.abs.error
	  endif
...
if crosstr_selection then begin
  ...
  ;Create errcrosstransforms structure
  if error then begin
    errcrosstransforms={abs:{value:fltarr(crosssize,transformsize(1),transformsize(2)),error:fltarr(crosssize,transformsize(1),transformsize(2))},$
  ...
  for i=0,crosssize-1 do begin
	...
	;Calculate errcrosstransforms
	if error then begin
	  ;Absolute value
	  errcrosstransforms.abs.value(i,*,*)=abs(crosstransforms(i,*,*))
	  errtransform1=reform(errtransforms.abs.error(where(channels EQ channelpairs_used(0,i)),*,*))
	  errtransform2=reform(errtransforms.abs.error(where(channels EQ channelpairs_used(1,i)),*,*))
	  errcrosstransforms.abs.error(i,*,*)=sqrt((abs(transform1)*errtransform2)^2+(abs(transform2)*errtransform1)^2)
	  ;Phase
	  errcrosstransforms.phase.value(i,*,*)=atan(crosstransforms(i,*,*), /phase)
	  errtransform1=reform(errtransforms.phase.error(where(channels EQ channelpairs_used(0,i)),*,*))
	  errtransform2=reform(errtransforms.phase.error(where(channels EQ channelpairs_used(1,i)),*,*))
	  errcrosstransforms.phase.error(i,*,*)=sqrt(errtransform1^2+errtransform2^2)
	endif
  endfor
endif
