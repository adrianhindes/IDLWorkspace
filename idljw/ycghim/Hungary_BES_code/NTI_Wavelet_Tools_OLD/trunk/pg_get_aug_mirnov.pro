...
if keyword_set(no_data) then begin
	...
	if where(shot EQ [19807,19821,20975,21292,22154,22188,22265,22268,22310,22375,22377,22382,$
	      22501,22559,22565,23418,23824,23913,25740,25845, 26941]) GT -1 then begin
  ...
  if (strcmp(strmid(channel,0,3),'C09') AND (where(chnum EQ $
	...
	if strcmp(diag,'') then begin
		print,'Diagnostic not defined for channel '+channel+' in shot '+nti_wavelet_i2str(shot)
		return,{t:[0,0],s:[0,0]}
	endif
