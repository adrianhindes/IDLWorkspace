...
;      [,freq_min] [,freq_max] $
;      [,crosstr_selection] [,coh_selection] [,coh_avr] $
...
if coh_selection then begin
  ; Create array for coherences and transfer functions
  coherences=fltarr(crosssize,transformsize(1),transformsize(2))
  transfers=complexarr(crosssize,transformsize(1),transformsize(2))
  coeffs=fltarr(crosssize,transformsize(1),transformsize(2))
  ...
  ;Calculate coherence and transfer function and coupling coefficients:
  for i=0,crosssize-1 do begin
    ...
    transfers(i,*,*)=complex(reform(smoothed_crosstransforms(i,*,*))/smoothed_apsd1)

    dt=double(timeax(n_elements(timeax)-1)-timeax(0))/double(n_elements(timeax)-1)
    coeffs(i,*,*)=couple_coeff(transf = transfers(i,*,*), a = smoothed_apsd1, b = smoothed_apsd2, sa = 1000, sb = 1000, n = coh_avr, $
	snr = 1d2, timew = dt*stft_length/stft_step)
  endfor
endif
