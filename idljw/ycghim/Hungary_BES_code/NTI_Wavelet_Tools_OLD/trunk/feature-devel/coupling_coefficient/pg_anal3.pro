...
for i=0,bsize[2]-1 do begin
	...
	if aplot then begin
		...
		title=title+' ('+i2str(nsegment)+' segments)'
		if poster then title='ACF '+i2str(b[0,i])+'-'+i2str(b[1,i])+' kHz'
...
for i=0,bsize[2]-2 do begin
	for j=i+1,bsize[2]-1 do begin
		...
		if cplot then begin
			...
			if print then device,filename=pg_filename(title) else window,/free
			title=title+' ('+i2str(nsegment)+' segments)'
			if poster then title='CCF '+i2str(b[0,i])+'-'+i2str(b[1,i])+' kHz - '+i2str(b[0,j])+'-'+i2str(b[1,j])+' kHz'
