...
;    crosstransforms, smoothed_crosstransforms, coherences, modenumbers, qs $
...
pro nti_wavelet_plot, $
  ; Inputs - calculation results
    ...
    coherences=coherences, transfers=transfers, coeffs = coeffs, modenumbers=modenumbers, qs=qs,$
...
; Set defaults
default,transf_selection,0
default,transf_smooth,0
default,transf_energy,0
default,transf_phase,0
default,transf_cscale,0.4
default,crosstr_selection,0
default,crosstr_smooth,0
default,crosstr_energy,0
default,crosstr_phase,0
default,crosstr_cscale,0.4
default,coh_selection,0
default,coh_all,0
default,coh_avg,0
default,coh_min,0
default,mode_selection,0
default,mode_cohlimit,0
default,mode_powlimit,0
default,mode_qlimit,100
default,linear_freqax,0
if not(defined(savepath)) then cd, current=savepath
if not(defined(startpath)) then cd, current=startpath
default,version,'Unidentified version'
...
;1 direction
  ...
  loadct,5,file=startpath+'nti_wavelet_colors.tbl'
    for i=0,crosssize-1 do begin
      printdatas=shot_printdatas+$
        ...
        transf_printdatas+$
        '!Caverages: '+i2str(coh_avr)
      plotted=reform(abs(transfers(i,*,*)))
  ...
  if mode_cohlimit NE 0. then begin
    ...
    if max(notplotted) GT -1 then plotted(notplotted)=0.6
  endif

      if cwt_selection then begin
        title='Transfer function of '+expname+' '+i2str(shotnumber)+'-'+channelpairs_used(0,i)+'--'+channelpairs_used(1,i)
        device,filename=pg_filename(title,dir=savepath)
        if linear_freqax then begin
          levels=findgen(60)
          levels=levels/max(levels)
          ...
          pg_plot4,reverse(plotted,2),xrange=[min(timeax),max(timeax)],yrange=[min(freqax),max(freqax)]$
            ,/ylog,y2range=[min(-pg_log2(scaleax)),max(-pg_log2(scaleax))]$
            ,xtitle='Time (s)',ytitle='Frequency (kHz)',y2title='-log base 2 of scale'$
            ,title=title, opt=crosstr_cscale, data=printdatas
        endelse
      endif else begin
        title='Transfer function of '+expname+' '+i2str(shotnumber)+'-'+channelpairs_used(0,i)+'--'+channelpairs_used(1,i)
        device,filename=pg_filename(title,dir=savepath)
        pg_plot4,plotted,xrange=[min(timeax),max(timeax)],yrange=[min(freqax),max(freqax)]$
          ,xtitle='Time (s)',ytitle='Frequency (kHz)',title=title,data=printdatas, opt=crosstr_cscale
      endelse
    endfor
;-------------------------------
;Plotting coupling coefficients!
;1 direction
  print,'Plotting coupling coefficients!'
  loadct,5,file=startpath+'nti_wavelet_colors.tbl'
    for i=0,crosssize-1 do begin
      printdatas=shot_printdatas+$
        '!Cchannels: !C '+channelpairs_used(0,i)+'--'+channelpairs_used(1,i)+$
        transf_printdatas+$
        '!Caverages: '+i2str(coh_avr)
      plotted=reform(abs(coeffs(i,*,*)))
      if cwt_selection then begin
        title='Coupling coefficients of '+expname+' '+i2str(shotnumber)+'-'+channelpairs_used(0,i)+'--'+channelpairs_used(1,i)
        device,filename=pg_filename(title,dir=savepath)
        if linear_freqax then begin
          levels=findgen(60)
          levels=levels/max(levels)
