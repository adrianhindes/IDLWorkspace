...
;INPUT(S):
  energy_density, timeax, freqax,crosstransforms, chpos,$
       errors, t_start=t_start, t_end=t_end, bandwidth=bandwidth, start_indeces_y=start_indeces_y,$
;OUTPUT(S):       
  cphase_time_dev = cphase_time_dev,$
;OPTIONAL:
  print=print
...
; cphase_time_dev(i,2,*) for difference between measured and expected values
    ...
    for i=first(0), last(0)-1  do begin
      ...
      cphase_time_dev(i,2,*)=phases_and_pos(2,*)
    endfor
  endif
endif
...
if print then begin
  ...
  for i=0, n2-1 do begin
    ...
    plot, timeax, cphase_time_dev(*,2,i), xstyle=1, title=title0, xtitle='Time [s]',$
      ystyle=1, ytitle='Difference [Pi]', charsize=3., thick=5
      device,/close
      wait, 0.2
  endfor
endif
