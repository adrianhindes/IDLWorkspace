...
;Note that 'list' is not a good name as it is a built-in function.
;For instance, bes_read_data.pro calls STRSPLIT function with an
;/extract keyword set for which 'list' is internally invoked.
;Once 'list' function is invoked, setting 'list' to a common block
;makes a compile time error. 
;Thus, I change the 'list' below to 'llist'.

  common loc_truecolor, llist
  ...
  if (n_elements(llist) eq 0) then begin
    ncol=141
    llist=replicate({name:'', hex:'', r:0B, g:0B, b:0B, rgb:0L}, ncol)
    tmp=strarr(ncol)
    ...
    rgb=llist.hex
    llist.hex=strmid(rgb, 4, 2)+strmid(rgb, 2, 2)+strmid(rgb, 0, 2)
    rgb=llist.rgb
    reads, llist.hex, rgb, format='(z)'
    llist.rgb=rgb
    
    llist.b=rgb/65536
    llist.r=rgb mod 256
    llist.g=(rgb/256) mod 256

    llist=temporary(shift(llist, 1))
  endif

  if keyword_set(setcolortable) or (!d.name eq 'PS') then begin
    tvlct, r, g, b, /get
    if ~array_equal([r,g,b], [llist.r,llist.g,llist.b]) then tvlct, llist.r, llist.g, llist.b
  endif

  if keyword_set(showlist) then return, llist(1:*).name
  ...
  for i=0, n_elements(name)-1 do begin
    pos=where(strpos(llist.name, name[i]) eq 0, flag)
  ...
  if (!d.name eq 'X') or (!d.name eq 'WIN') then begin
    ...
    if (decompsetting) then return, llist[rgb].rgb
  endif
