...
; read the dafault values from show_bes_data_widget.def
  openr, lun, 'show_bes_data_widget.def', /get_lun
  line = ''
...
; create widget
  ...
  window_title = 'Show BES data v.' + strcompress(string(version, format='(f0.1)'), /remove_all)
  ...
  shot_number_base = widget_base(base, /row, xsize = xsize, ysize = 30)
    ...
    shot_number_text = widget_text(shot_number_base, value = baseinfo.shotnumber, /editable, xsize=10)
  ...
  ch_base = widget_base(base, /column, xsize = xsize, frame = 1)
    ...
    ch_base2 = widget_base(ch_base, /column, xsize = xsize)
      ch_base21 = widget_base(ch_base2, /row, xsize = xsize, ysize = 20)
        bes_ch_label = widget_label(ch_base21, value = '            8     7     6     5     4    3     2    1 ')
    for j=0, 3 do begin
      ch_base22 = widget_base(ch_base2, /row)
        ...
        for i=0, 7 do begin
          bes_ch_button[((4-j)*8-1)-i] = widget_button(ch_base221, value=' ')
          widget_control, bes_ch_button[((4-j)*8-1)-i], set_button = baseinfo.bes_ch_sel[((4-j)*8-1)-i]
        endfor
    endfor
