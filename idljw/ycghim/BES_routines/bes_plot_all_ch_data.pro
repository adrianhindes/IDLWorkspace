...
;******************************************************************************************************
  ...
  default, old_bes_system_shot_end, 9427 ;8x4 channel system up to shot#9427, and 16x4 channel system after.

  if shot LE old_bes_system_shot_end then begin
    nrow = 8
    ncol = 4
  endif else begin
    nrow = 16
...
; plot the data
  ...
  window, /free, xs=1600, ys=800
  for j=0, ncol - 1 do begin
    for i=0, nrow - 1 do begin
      if shot LE old_bes_system_shot_end then begin
        inx_radial = nrow-i-1
        inx_vertical = ncol-j-1
      endif else begin
        inx_radial = i
        inx_vertical = j
      endelse
 ...
 ;     plot, time, data[nrow-i-1, ncol-j-1, *], xr=xrange, yr=yrange, $
      ...
      title = STRING(shot, format='(i0)') + ': ' + STRING(inx_vertical+1, format='(i0)') + '-' + STRING(inx_radial+1, format='(i0)')
      plot, time, data[inx_radial, inx_vertical, *], xr=xrange, yr=yrange, $
