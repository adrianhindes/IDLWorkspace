...
;*     ctable    : color table to use [default=5]                                                    *
;*                                                                                                   *
...
;*         .nlevels: number of levels used in the contour plot (default=20)                          *
...
pro cw_draw2d_extralegend, legend_settings, charsize_in=charsize_in
  ...
  if ~tag_exists(legend_settings,'text') then return else text=textoidl(legend_settings.text)
  ...
  if tag_exists(legend_settings,'linestyle') then begin
    legendd, text, color=color, textcolor=textcolor,linestyle=legend_settings.linestyle,psym=psym,$
    ...
    if psym eq 0 then begin
      legendd, text, textcolor=textcolor, thick=thick, box=box, clear=clear, horizontal=horizontal,$
                    spacing=spacing, charthick=charthick, charsize=charsize,$
                    top=top,bottom=bottom,left=left,right=right
    endif else begin
      legendd, text, textcolor=textcolor,psym=psym, color=color,$
                    symsize=symsize, thick=thick, box=box, clear=clear, horizontal=horizontal,$
                    spacing=spacing, charthick=charthick, charsize=charsize,$
                    top=top,bottom=bottom,left=left,right=right
    endelse
...
pro cw_draw2d_datax, state
      ...
      if state.ndata gt 1 then begin
        for j=1,state.ndata-1 do begin
          ...
          if cw_draw2d_check(odata) eq -1 then yidx=0 else begin
            tmp=min(abs(odata.y - data.y[sticky.yidx[i]]), yidx)
          endelse
  ...
  if state.ndata gt 1 then begin
    for j=1,state.ndata-1 do begin
      ...
      ; get the yidx in odata closest to yidx in data
      if cw_draw2d_check(odata) eq -1 then yidx=0 else begin
        tmp=min(abs(odata.y - data.y[info.yidx]), yidx)
      endelse
  ...
  if cw_draw2d_check(data) eq 1 then $
     legendd, lgdtxt, textcolors=[lincol,!p.background],charsize=charsize,$
...
pro cw_draw2d_contour, state
  ...
  widget_control, id.contopt.nlevels, get_value=nlevels
  nlevels=fix(nlevels[0])
  if nlevels gt 0 then begin
    if nlevels gt 99 then nlevels=99
  ...
  if nlevels le  0 then widget_control, id.contopt.nlevels, set_value='0'
  if nlevels ge 99 then widget_control, id.contopt.nlevels, set_value='99'
...
pro cw_draw2d_surf, state
  ...
  cp_surf, ztmp, xtmp, ytmp, ax=info.ax, az=info.az,$
            xrange=info.xrange, yrange=info.yrange, zrange=info.zrange,$
            xtitle=xtitle, ytitle=ytitle, ztitle=ztitle, title=title, charsize=charsize,$
            /showscale, cvalues=cvalues, clabel=clabel, ctable=info.ctable
  ...
  widget_control, id.crosshair, get_value=crossh
  if crossh[0] then begin
    plots, [data.x[info.xidx],data.x[info.xidx]],info.yrange,[info.zrange[1],info.zrange[1]],$
           /data,/T3D,col=truecolor('blue'),linestyle=2, noclip=0
    if size(data.y, /n_dim) le 1 then begin  ; y-dimension is constant
      plots, info.xrange,[data.y[info.yidx],data.y[info.yidx]],[info.zrange[1],info.zrange[1]],$
             /data,/T3D,col=truecolor('blue'),linestyle=2, noclip=0
    endif else begin                         ; y-dimension is x-dependent
      plots, info.xrange,[data.y[info.xidx,info.yidx],data.y[info.xidx,info.yidx]],$
             [info.zrange[1],info.zrange[1]],$
             /data,/T3D,col=truecolor('blue'),linestyle=2, noclip=0
    endelse
    if size(data.y, /n_dim) le 1 then begin  ; y-dimension is constant
      plots, [data.x[info.xidx],data.x[info.xidx]],[data.y[info.yidx],data.y[info.yidx]],$
             [info.zrange[1],data.z[info.xidx,info.yidx]],/data,/T3D,col=truecolor('blue'),$
             linestyle=2, noclip=0
    endif else begin                         ; y-dimension is x-dependent
      plots, [data.x[info.xidx],data.x[info.xidx]],$
             [data.y[info.xidx,info.yidx],data.y[info.xidx,info.yidx]],$
             [info.zrange[1],data.z[info.xidx,info.yidx]],/data,/T3D,col=truecolor('blue'),$
             linestyle=2, noclip=0
    endelse
...
pro cw_draw2d_setvalue, base, data
  ...
  if (size(data,/type) gt 1) && (size(data,/type) lt 6)  && (data[0] eq -1) then begin
    ...
    widget_control, id.contopt.nlevels, set_value=string(format='(I0)',20)
  ...
  if ~(cw_draw2d_check(data) eq 0) then begin
    ...
    ;              to the data, set ndata to 1, update ranges, plot type, .... et cetera
      ...
      ; set the default number of contours
      if cw_draw2d_check(data,istag='nlevels') then nlevels=abs(fix(data.nlevels mod 100)) else nlevels=10
      widget_control, id.contopt.nlevels, set_value=string(format='(I0)',nlevels)
...
pro cw_draw2d_setcolor_event, state
  ...
  widget_control, id.draw, get_uvalue=info
  ; get the color table
  ctable = widget_info(id.contopt.color,/droplist_select)
  ; set the color table
  info.ctable = ctable
  widget_control, id.draw, set_uvalue=info
end
...
function cw_draw2d_event, event
  ...
  id.contopt.nlevels: begin & return_event=0 & replot=1 & end
...
function cw_draw2d, base, xsize=xsize, ysize=ysize, frame=frame, event_pro=epro, background=background,$
  ...
  id.contopt.lbllevel = widget_label(id.contopt.top, value='#contours:', /align_left)
  id.contopt.nlevels  = widget_text(id.contopt.top, value='20', /editable,xsize=2)
  ...
  drawcol   = widget_base(drawrow,/col)
  id.draw   = widget_draw(drawcol,xsize=xsize,ysize=ysize, /button_events, retain = 2)
  ...
  if ~keyword_set(ctable) then ctable=5l
  widget_control, id.contopt.color, set_droplist_select=ctable
