...
; seen from the top is replaced by a 'normal, 3D' surface mesh. This surface
; is shown in a box, with (if the keyword is set) a contour plot at the bottom.
; The title is placed in the left-hand corner (because according to my own
; humble opinion the standard title printed by 'surface' looks ugly).
;
;---------------------------------------------------------------------------
; Based on Routine: cp_shade 
; Version: 1.20
; Author : R.Martin
; Date   : 8.12.03
;---------------------------------------------------------------------------
...
;    
  ...
  if is_real(zrange) then begin
    ...
    ; data cropped to zrange (otherwise the surface plot looks messy)
    idx = where(idata ge zmax,cnt)
    if cnt ne 0 then idata[idx]=zmax

    idx = where(idata le zmin,cnt)
...
;       SCALE_POSTION option
  ...
  if keyword_set(showscale) then begin
    ...
    if (scalelabel ge 2) then begin
      ...
      if scalelabel eq 2 then plotpos(2) = plotpos(2)- 6.*chwidth
      bx=[0,1]
      by=[zmin, zmax]
      bdata=[1,1]#by
    endif else begin
...
;Add shade-surface plot
  ...
  if keyword_set(title) && is_string(title) then begin
    xyouts, 0.0, 0.0, ' ', /normal, width=chwidth, charsize=!p.charsize*1.3
    xyouts, 2.0*chwidth,1.0-5.0*chwidth, /norm, title, charsize=!p.charsize*1.3
  endif
