...
print, 'cp_shade version by mdebock....'
...
;     - If no data is in the given YRANGE a blank plot is shown
  ...
  if is_real(yrange) then begin
    ...
    if (ymin eq ymax) then begin
      error_message='CP_SHADE: Y-Range badly defined (xmin=xmax)'
      goto, errorcatch
    endif
...
;       SCALE_POSTION option
  ...
  if keyword_set(showscale) then begin
    ...
    if (scalelabel ge 2) then begin
      ...
      if scalelabel eq 2 then plotpos(2) = plotpos(2)- 7.*chwidth
      bx=[0,1]
      by=[zmin, zmax]
      bdata=[1,1]#by
    endif else begin
...
;      else xtickpos=scalepos[2]+chwidth*3.5
