function read_segmented_images, tree, shotno, node, index, long=long, all=all, bin=bin, status=status
...
mdsopen, tree, shotno
  ...
  if nframe eq 1 then begin
    ...
    for i = 0, n_elements(index)-1 do begin
      idx = index[i]
      images[*,*,i] = rebinb(mdsvalue('GetSegment('+ node +', $)', idx, status=status), bin)
